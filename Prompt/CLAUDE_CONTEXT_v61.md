# CLAUDE_CONTEXT_v61.md — Orbit OMS
# Consolidated from v38 + all context updates through v60
# Schema: v26.3
# April 2026
...
### 6.9 Keyboard Shortcuts (v61)

| Key | Action |
|---|---|
| Ctrl+C | Smart copy state machine (FIXED v61) |
| Ctrl+V | Auto-paste into SO Number input (FIXED v61) |
| E | Open Slot Email modal (was Ctrl+M, changed v61) |
...
**Changed in v61:** Ctrl+M → E (single key). Ctrl+C/V fixed (separate useEffect with stopImmediatePropagation).
...
**Trigger:** Auto-trigger 15min after slot cutoff. Manual via E key (changed from Ctrl+M in v61). localStorage key `mo-slot-email-sent-{date}-{slotName}` prevents re-trigger. Auto-trigger guard: `triggered` flag ensures only one slot fires at a time.
...
17. **Keyboard handler architecture:** Ctrl+ shortcuts MUST be in a separate `useEffect` from single-key shortcuts. Ctrl+ handler uses `document` capture + `stopImmediatePropagation`. Single-key handler early-returns on `e.ctrlKey || e.metaKey`. This prevents any other component's capture listener from consuming Ctrl+ events.
...
25. **Order detail link in email** — SO number in slot email → orbitoms.in/orders/{soNumber}. Needs public order detail page.
26. **senderPhone hardcoded placeholder** — in email-template.ts.
27. **Day summary email** — Ctrl+D trigger, exception-only format. Not built.
...
1. You have read CLAUDE_CONTEXT_v61.md fully
...
12. **Mail Order keyboard (v61):** Ctrl+C=smart copy, Ctrl+V=paste SO, E=slot email, R=reply, F=flag, S=SKU panel, N=next unmatched, P=pick, T=toggle punched, L=order list, /=search, Esc=cascade. Q/W/A removed. Ctrl+M deprecated.
...
- **Ctrl+M shortcut** — replaced by single-key `E` in v61.

---

*Version: Phase 1 Go-Live · Schema v26.3 · Context v61 · April 2026*
