...
# Command: claude "Read CLAUDE_CONTEXT_v21.md fully before doing anything else."
...
### Auto-import API endpoint (NEW v21)
- Route: `POST /api/import/obd?action=auto`
- Auth: HMAC-SHA256 — no session required. Verified in `verifyHmacSignature()`.
- HMAC method: `createHmac("sha256", secret).update("auto-import-v1").digest("hex")`
- Secret stored in: Vercel env var `IMPORT_HMAC_SECRET` (server) + `import-key.txt` (PowerShell)
- Headers required: `x-import-key-id: auto-import-v1` + `x-import-signature: <hmac-hex>`
- Always uses `combined_v2` template — reads `LogisticsTrackerWareHouse` + `LineItems` sheets
- Runs full preview + confirm in one shot — no human review step
- Uses `importedById: 1` (admin user) as system user placeholder
- Returns: `{ success, batchRef, ordersCreated, skippedDuplicates, errors }`
- Middleware bypass: requests with `x-import-key-id` header on `/api/import/obd` skip NextAuth redirect
...
```
OBD-Import Tool\
    Merge-OBD-Combined.ps1       <- manual merge script (run on demand)
    Auto-Import.ps1              <- NEW v21. auto merge + push to API (Task Scheduler)
    import-key.txt               <- HMAC secret key (one line, plain text, never commit)
    ...
    logs\                        <- daily log files: import-log-YYYY-MM-DD.txt (30 day retention)
    Master\
...
### Auto-Import script (NEW v21)
Script: `Auto-Import.ps1`
Purpose: Fully automated — merges files AND pushes to OrbitOMS API without human intervention.
Scheduled via Windows Task Scheduler: every 10 minutes, 8AM-8PM daily.
Logs every run to `logs\import-log-YYYY-MM-DD.txt`.

Authentication: HMAC-SHA256 of fixed string `"auto-import-v1"` using shared secret from `import-key.txt`.
No timestamp — simple shared secret approach. Secure because secret never travels in plain text.

Log format:
```
[YYYY-MM-DD HH:MM:SS] RUN STARTED - N header file(s), N OBD file(s)
[YYYY-MM-DD HH:MM:SS] MERGED - N header rows, N line items -> Combined_OBD_YYYY-MM-DD.xlsx
[YYYY-MM-DD HH:MM:SS] UPLOADING - https://orbitoms.in/api/import/obd?action=auto
[YYYY-MM-DD HH:MM:SS] SUCCESS - batchRef=BATCH-... | imported=N | skipped=N | errors=N
[YYYY-MM-DD HH:MM:SS] RUN COMPLETE
```
...
/lib
  ...
  import-types.ts                   — ImportPreviewResponse, ImportConfirmResponse types
/prisma
...
| Phase 3.9 — Import Template System + Auto-Import | Complete |
...
68. Auto-import endpoint `?action=auto` bypasses session auth — uses HMAC only. Check middleware.ts for bypass logic.
69. HMAC signs fixed string `"auto-import-v1"` — NO timestamp. Simple shared secret approach.
70. `verifyHmacSignature()` reads headers `x-import-key-id` and `x-import-signature` (all lowercase — Edge runtime lowercases headers)
71. Middleware exports plain `middleware` function (not `export default auth(...)`) — allows header check BEFORE NextAuth runs
72. `new URL(req.url, "http://localhost")` used in POST handler — fixes relative URL parsing on Vercel Edge
73. Auto-import always uses `combined_v2` template — hardcoded in `handleAutoImport`, no templateId from FormData
74. `import-key.txt` on dedicated PC must NEVER be committed to git — contains the shared HMAC secret
75. Task Scheduler: OrbitOMS Auto Import — runs every 10 min, 8AM-8PM, "Run only when user is logged on"
...
*Version: Phase 3 · Schema v14 · Context v21 · Import Templates v1 · Auto-Import v1 · TM Table View v1 · Config Master v2 · Operator Screen v5 · TI Report v2 · March 2026*
