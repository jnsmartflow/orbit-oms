...
# Command: claude "Read CLAUDE_CONTEXT_v30.md fully before doing anything else."
# Version: Phase 4 Warehouse Board Design Locked · Schema v18 · Context v30 · March 2026
...
## 36. Session Start Checklist (UPDATED v30)
...
## 37. Warehouse Board UI Components (NEW v30)

Page: /warehouse
Roles: Floor Supervisor, Admin

### Implementation Status: NOT STARTED — Design locked at v8

### Components planned:

| Component | Purpose |
|---|---|
| warehouse-page.tsx | Main orchestrator — types, state, API calls, auto-sort logic |
| warehouse-header.tsx | Header: title, date, refresh, stats (Unassigned/Picking/Picked/OBDs) |
| warehouse-delivery-tabs.tsx | Reuse or clone from planning — delivery type tabs |
| warehouse-slot-tabs.tsx | Underline-style slot tabs (NOT cards) with pick progress X/Y |
...
| pickers-panel.tsx | Right panel: picker lanes container |
| picker-lane.tsx | Expandable lane: header row + pending cards + done chip |
...
- `GET /api/warehouse/board` — orders for warehouse view: unassigned + assigned, grouped by customer, includes pick status, tinting, vehicle/trip info
- `GET /api/warehouse/pickers` — all picker-role users with current assignment counts and pick stats for the day
- `POST /api/warehouse/assign` — assign orders to a picker: body { orderIds[], pickerId }, creates pick_assignments with sequence numbers
- `POST /api/warehouse/orders/[id]/mark-picked` — mark OBD as picked (sets isPicked, pickedAt, pickedById on orders table)
