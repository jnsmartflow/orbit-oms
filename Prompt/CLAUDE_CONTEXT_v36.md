...
role_master                — 8 roles (added 'operations' role in v36, id=12)
...
users                      — Depot staff accounts
                             Columns: id, email, password, name, roleId, isActive, createdAt, updatedAt
                             NOTE: password hashed with bcryptjs 10 rounds
                             NOTE: roleId FK -> role_master.id (NOT slug)
```
...
61. **Role redirects FIXED (v36):** dispatcher→/planning, floor_supervisor→/warehouse, picker→/warehouse
62. **Stub pages (v36):** /dispatcher, /warehouse/supervisor, /warehouse/picker now server-redirect to correct URLs
63. **Operations role (v36):** role_master id=12, slug='operations', user operations@orbitoms.com (pw: operations123 — change in prod)
64. **Operations routes (v36):** /operations/support|tinting|tint-operator|dispatch|warehouse — each renders existing board component
...
## 42. Known Issues / Pending Fixes (UPDATED v36)
...
## 43. Queued Features (UPDATED v36)
...
## 48. Operations Role (NEW v36)

### Role & User
- `role_master`: id=12, name='operations'
- `users`: email=operations@orbitoms.com, name='Operations User', roleId=12
- Temp password: `operations123` — **must be changed before production use**
- Password hashed with bcryptjs 10 rounds

### Routes
```
/operations                    → redirects to /operations/support
/operations/support            → <SupportPageContent />
/operations/tinting            → <TintManagerContent />
/operations/tint-operator      → <TintOperatorContent />
/operations/dispatch           → <PlanningPage />
/operations/warehouse          → <WarehousePage />
```

### Layout
- `app/(operations)/layout.tsx` — passthrough
- `app/(operations)/operations/layout.tsx` — auth check (operations|admin), builds nav via getAllPermissionsForRole, wraps in RoleSidebarProvider + RoleLayoutClient

### Sidebar nav (role_permissions rows for roleSlug='operations')
| pageKey | href |
|---|---|
| operations_support | /operations/support |
| operations_tinting | /operations/tinting |
| operations_tint_operator | /operations/tint-operator |
| operations_dispatch | /operations/dispatch |
| operations_warehouse | /operations/warehouse |

### API access
ROLES.OPERATIONS added to requireRole() in 38+ route files:
- All app/api/support/* routes (11 files)
- All app/api/planning/* routes (8 files)
- All app/api/tint/manager/* routes (14 files)
- All app/api/tint/operator/* routes (5 files)
- All app/api/warehouse/* routes (3 files)
- app/api/orders/[id]/detail/route.ts
- app/api/operations/summary/route.ts
NOT included: app/api/admin/* routes (admin-only)

### Operations Summary API
- Route: `GET /api/operations/summary`
- Returns: import counts, tinting counts, dispatch counts, warehouse counts, alerts
- Used by: `components/admin/operations-overview.tsx` (kept for future use)
- Tinting status values: pending=['tint_assigned','pending'], inProgress='tinting_in_progress', done='tinting_done'

### lib/rbac.ts
- Added: `OPERATIONS: 'operations'`

### lib/permissions.ts PAGE_NAV_MAP pageKeys added
- operations_support, operations_tinting, operations_tint_operator, operations_dispatch, operations_warehouse

---

## 49. Role Navigation Fixes (NEW v36)

### Post-login redirects (app/login/page.tsx ROLE_REDIRECTS)
| Role | Redirect |
|---|---|
| admin | /admin |
| dispatcher | /planning |
...
### Stub pages → server redirects
- `app/(dispatcher)/dispatcher/page.tsx` → redirect('/planning')
- `app/(warehouse)/warehouse/supervisor/page.tsx` → redirect('/warehouse')
- `app/(warehouse)/warehouse/picker/page.tsx` → redirect('/warehouse')
