...
# Command: claude "Read CLAUDE_CONTEXT_v34.md fully before doing anything else."
# Version: Phase 4 Import Fix · Schema v21 · Context v34 · March 2026
...
## 42. Known Issues / Pending Fixes (UPDATED v34)
...
7. ~~**Import not working**~~ — **FIXED v34.** Root cause: slotId never set during import. See §44.
8. **Slot cascade cascades pending_support orders** — cascade moves ALL orders from closed slots, including those not yet submitted by Support. Consider adding workflowStage/dispatchStatus filter to cascade eligibility.

---

## 43. Queued Features (UPDATED v34)

- ~~**Slot cascade**~~ — **DONE v33**
- ~~**Import debugging**~~ — **DONE v34** — all import issues fixed
- ~~**OBD date parsing fix**~~ — **DONE v34** — time merged into obdEmailDate as IST→UTC
- **Cascade badge** — NEW v34: When `originalSlotId !== slotId`, show `⏩ from {originalSlot.name}` badge on order rows. All views (Support, Planning, Warehouse). Data already available — purely UI work.
...
## 44. Import Fix Session Log (NEW v34)

### Root cause diagnosis

Import pipeline (manual + auto) created orders successfully but they were **invisible** in Support, Planning, and Warehouse boards. Root cause: orders were created with `slotId: null` because `resolveSlot()` only returned the slot **name** (legacy string) but never the slot **id** (FK to slot_master). All downstream views filter/group by `slotId`.

### Fixes applied (5 changes across 3 files)

#### Fix 1: slotId + originalSlotId set during import
**File:** `app/api/import/obd/route.ts`
- Added `id` to `SlotConfig` type and slot select queries
- `resolveSlot()` now returns `slotId: number | null` alongside `dispatchSlot` string
- Both `handleConfirm` and `handleAutoImport` set `slotId` and `originalSlotId` on order creation
- Both slot config queries (`delivery_type_slot_config`) now include `slot: { select: { id: true, ... } }`

#### Fix 2: obdEmailDate time merge (IST→UTC)
**File:** `app/api/import/obd/route.ts`
- New helper function `mergeEmailDateTime(emailDate, emailTime)` 
- Combines `obdEmailDate` (date only, midnight UTC) with `obdEmailTime` (IST string like "10:30")
- Converts IST to UTC: subtracts 5h30m before storing
- Result: `obdEmailDate` stores full timestamp (e.g. `2026-03-31T05:00:00Z` for 10:30 IST)
- Applied in both `handleConfirm` and `handleAutoImport`

#### Fix 3: smu column on orders
**Schema:** `ALTER TABLE orders ADD COLUMN smu TEXT` via Supabase SQL Editor
**Prisma:** Added `smu String?` to orders model in `schema.prisma`
**File:** `app/api/import/obd/route.ts`
- Both `handleConfirm` and `handleAutoImport` now set `smu: summary.smu` on orderData

**File:** `components/support/support-orders-table.tsx`
- Added `smu: string | null` to `SupportOrder` interface
- Fixed `getSmuGroup()` — was hardcoded to return "Decorative Projects" / "Retail Offtake", now reads `order.smu`

#### Fix 4: Support slot assignment saving
**File:** `components/support/support-orders-table.tsx`
- `handleSubmitSelected()` only processed dispatch/hold edits — **ignored slot changes**
- Added slot change collection and `onAssignSlot()` calls inside `handleSubmitSelected`
- Slot changes applied before dispatch/hold status changes

#### Fix 5: Auto-import duplicate data prevention
**File:** `app/api/import/obd/route.ts`
- Auto-import previously wrote `import_raw_summary` + `import_raw_line_items` for ALL OBDs including duplicates
- Now skips duplicate OBDs entirely (`continue` before writing raw data)
- Duplicate count computed from `existingObdSet` instead of `insertedSummaries`
- Result: repeat auto-import runs complete instantly with `skippedDuplicates: N`

#### Fix 6: Cascade wired into Support slots API
**File:** `app/api/support/slots/route.ts`
- Support page now triggers `runDailyCleanupIfNeeded()` + `runSlotCascadeIfNeeded()` on load
- Same pattern as Planning and Warehouse boards — cascade consistent across all three views
- Throttled to 5-min intervals via `system_config.last_cascade_check`

### Import pipeline flow (post-fix)

```
XLS file → parseDateCell (date) + parseTimeCell (time)
         → import_raw_summary (stores both separately)
         → resolveSlot() returns { dispatchSlot, dispatchSlotDeadline, slotId }
         → mergeEmailDateTime() combines date + time as UTC
         → orders.create({ slotId, originalSlotId, smu, obdEmailDate: merged })
         → order visible in Support (grouped by slotId) → Planning → Warehouse
```

### Task Scheduler settings for Auto-Import
- General: "Run whether user is logged on or not" + "Run with highest privileges"
- Triggers: Weekly Mon-Sat, 8AM start, repeat every 10 min, duration 12 hours
...
#### Fix 6: Cascade wired into Support slots API
**File:** `app/api/support/slots/route.ts`
- Added `runDailyCleanupIfNeeded()` and `runSlotCascadeIfNeeded()` calls before slot count queries
- Same pattern as Planning and Warehouse board APIs
- Now all three views (Support, Planning, Warehouse) trigger cascade consistently
- Cascade still throttled to 5-min intervals via `system_config.last_cascade_check`
