...
# v111 · Schema v27.41 · September 2026 · updated 2026-09-24 · Lives in: orbit-oms/docs/
...
- **OBD pipeline:** SAP import (XLS / JSON auto-import / clipboard paste) → a non-tint bill with no mail order is released straight to picking at import (`applyNoMailOrderFallback`, owner `CLAUDE_IMPORT.md §2.1`); a tint bill goes through tinting and releases itself on completion (`CLAUDE_TINT.md §2`); a held bill waits on the Floor desk (`/floor`, `CLAUDE_FLOOR.md`) → picking (`/picking`) → loads planned as trips on the Floor trip desk (`CLAUDE_FLOOR_TRIPS.md`) → vehicle dispatch
...
- **A status STRING is not an enum — never retype the literal, import the vocabulary.** Most status columns here (`tint_assignments.status`, `order_splits.status`, `mrn.status`, `ci_returns.status`, `pick_assignments.status`) are plain Prisma `String`s. Several carry no database CHECK at all, so **a wrong literal is never rejected — it silently matches nothing**, and a filter written on it looks like it works forever. This is not hypothetical: `status: { not: "done" }` sat in FOUR live tint routes for over a year meaning "not finished", while the finished value was `tinting_done` and `"done"` had never been written once — so the predicate matched every row ever created (found and fixed 2026-09-05/06; account in `CLAUDE_TINT.md §1.4`). It had swept dead `skipped` rows into a `cancelled` `updateMany` and put one OBD in two operators' reorder queues. **The rule:** each module's status vocabulary gets ONE exported constant (`lib/tint/assignment-status.ts` is the reference implementation, naming each value's write site), and every route imports it. A hand-typed status string in a `where` is a defect waiting for the row that would have matched.
...
**Database runtime settings — applied to production 2026-09-08, written down here for the first time.** All three were set at **ROLE** level via the Supabase SQL Editor, and the role level is not a stylistic choice: **`ALTER DATABASE … SET` is REFUSED on Supabase** (permission denied — you do not own the database), and the alternative Supabase documents needs the Supabase CLI with `--experimental`. `ALTER ROLE` works and is the practical route.

| Setting | Applied to | Value | Why |
|---|---|---|---|
| `log_min_duration_statement` | `postgres`, `authenticator` | `'2000'` | Slow queries (>2s) are now logged. Before this, nothing recorded which query was slow. Both roles, because the app reaches Postgres as one and PostgREST as the other. |
| `statement_timeout` | `postgres` | `'30s'` | 🔴 **Before this the app's role had NO timeout at all**, while `anon`/`authenticated`/`authenticator` carried 3s/8s/8s. An app query could run unbounded — **a 300-second one appears in the 2026-09-07 outage log**. |

> **Escape hatch for maintenance.** A long migration or backfill will now be killed at 30s. Run
> `SET statement_timeout = 0;` **first, in the same SQL Editor tab** — it is session-scoped, so it
> applies to the statements that follow it in that tab and nothing else.

🔴 **LANDMINE — restarting Supabase WIPES `pg_stat_statements`.** The view is in-memory; a restart
resets it to empty and there is no warning and no recovery. On **2026-09-07 the restart destroyed the
only record of which query was slow during the outage, and the outage is unexplained as a direct
result** — the diagnosis was thrown away by the act of trying to fix the problem. **Capture the query
diary BEFORE any restart**, not after: `SELECT query, calls, total_exec_time, mean_exec_time, rows
FROM pg_stat_statements ORDER BY total_exec_time DESC LIMIT 50;` into a file. Same discipline applies
to `pg_stat_user_tables` counters, which also reset. Restarting is often the right move — just never
the *first* one.
...
| `Pull-TripReport.ps1` (added to this table 2026-09-08 — it had never been listed) | `C:\Users\HP\OneDrive\Automation\Orbit\nts trip report\` — ⚠ **OneDrive-synced**, so `settings.txt`'s service-role key and NTS credentials replicate to Microsoft's cloud | **LIVE** — a hand-opened window, **not** a scheduled task (verified 2026-09-08); loops every **61-65s**, not 60s | NTS trip mirror → `mirror_trip_report_today`. 🔴 **The host actually running it is UNKNOWN** — not this machine, which has no such process and a log dead since 2026-09-01 while production stays 60s fresh. **No `.ps1` change can be deployed until it is named.** ⚠ An obsolete pre-2026-07-06 copy also sits at `C:\Users\HP\OneDrive\VS Code\nts trip report\nts trip report\` and must not be run. Both: `CLAUDE_TRIP_REPORT.md §7` |
...
**Monitoring:** `/api/health` for manual checks. `vercel.json` currently defines **2 daily cron jobs** — attendance rollover (`35 18 * * *`) + photo purge (`30 20 * * *`), both `/api/cron/*`, bearer-auth via `lib/cron-auth.ts` (re-read in `vercel.json` 2026-09-19: unchanged).

**`next.config.mjs` rewrites and redirects:** `/demo` → `/order-demo.html`, and **`/.well-known/assetlinks.json` → `/assetlinks.json`** (Android Digital Asset Links for the TWA: a `public/.well-known/` folder is not served, so the file sits at a normal path and the required URL is rewritten onto it — the config comment records the 2026-08-12 live 404 that proved it; do not move it back). Redirects: `/tint/manager/ti-report` and `/ti-report` → `/reports?r=ti-report`.
...
>
> `system_config` key **`ACCESS_SOURCE`**. **Live value: `user`** (SELECT-verified 2026-09-04; re-read live 2026-09-18, Q02).
>
> ```sql
...
>
> `user_roles` (29 rows, 20 users) and `role_master` (13 roles — ids 1-7 and 12-17, live 2026-09-18 Q07a) are likewise retained. They still
> feed the label, the redirect and the `/admin/access` "what would their job title give them"
> baseline.
>
> ### Admin is now a flag
>
> **`users.isSuperuser`**, checked by 70 call sites in 60 files (grep for `isSuperuser(` /
> `requireSuperuser(` outside `lib/rbac.ts`, 2026-09-19; 68 on 2026-09-04) as **`isSuperuser === true` OR `roles includes
...
> What shipped and what did not: `docs/prompts/archive/2026-09/code-update-2026-09-04-user-based-access.md`.
...
| 15 | operation_manager | `/tint/manager` | Prakash (id=32, active). **CONFIRMED REAL 2026-08-04** (role_master SELECT: id 15; one active user) — settling the 2026-07-10 "undocumented slug, no confirmed row" open item; first flagged confirmed in `docs/prompts/archive/2026-08/web-test-plan-2026-07-29-picking-first-real-login.md`. |
| 17 | floor_access | none — not in `ROLE_REDIRECTS` (`lib/rbac.ts:23-45`) | Held only as a **secondary** role via `user_roles`: Ajay Vansiya (#29), Dhanraj Shah (#30), Priya Chaudhari (#31), Prakash (#32) — live 2026-09-18 Q07a/Q07b. A role slug, NOT the `floor` page key; in user mode the `floor` tick is what grants the page (`CLAUDE_FLOOR.md §1`). |
...
**Permissions:** `lib/permissions.ts` has `PAGE_NAV_MAP`, the **`PageKey`** union (`:205-320`), and `ALL_PAGE_KEYS` (`:353-375`) — **39 keys**. Every name in this paragraph is a member of the `PageKey` union, not a role slug and not a `RoleSidebarRole`. Twelve keys joined after the 2026-09-04 cutover's 27; the billing, tint-panel, report and ship-to ones are TABS or CONTROLS inside a screen, so none of them is in `PAGE_NAV_MAP`:

| `PageKey` | What it gates (code) | Owner |
|---|---|---|
| `billing_picking` | The Billing **Picking** tab — the five `/api/billing/picking/*` routes (canView on list/marker/order, e.g. `list/route.ts:92`; canEdit on mark-done/undo) and the tab in the mail-orders layout (`app/(mail-orders)/mail-orders/layout.tsx:70`). ⚠ NOT the floor board's `picking` | `CLAUDE_BILLING.md §4/§6` |
| `billing_print` | The Billing **Print** tab (slice 9, `22ced2d8`) — canView on `/api/billing/print/list:39` and `/marker:30`, canEdit on `/trip/[id]/copy:34`; layout `:77` | `CLAUDE_BILLING.md §7` |
| `billing_hold` · `billing_slot` · `billing_urgent` · `billing_ship_to` | One per button on the Billing Orders tab; `canEdit` is their only meaning. **Read** by `POST /api/billing/mail-order/actions` (`ACTION_KEY` map `:122-127`, check `:136`, repointed `a991a1c4`) and by the layout (`:96-99`). They scope the Billing face only; Floor sets the same facts on the `floor` key | `CLAUDE_BILLING.md §5` |
| `place_order_ship_to` | The Ship To block in the desktop `/place-order` cart — canEdit, read in `app/(place-order)/layout.tsx:47`. `/po` is untouched | `CLAUDE_PLACE_ORDER.md §3` |
| `tint_panel_items` · `tint_panel_details` · `tint_panel_activity` | The three Tint Manager job-panel tabs — canView, read in `app/(tint)/tint/manager/layout.tsx:38-40`. Server side `tint_panel_details` gates `/api/orders/[id]/audit-history:33`; `tint_panel_activity` gates the pause-history (`:26`) and skip-history (`:27`) routes; `tint_panel_items` is UI-only | `CLAUDE_TINT.md §13` |
| `reports_tint_summary` | The Tint Summary report — `/api/reports/tint-summary:37`, `app/reports/tint-summary/page.tsx:59` | `CLAUDE_TINT.md §12` |
| `reports_ti_report` | The TI Report — `/api/tint/manager/ti-report:24`; canExport = the Download Excel button (`app/reports/page.tsx:44`, UI-only) | `CLAUDE_TINT.md §11` |

Live `user_page_access` holders, 2026-09-18 (Q03a/Q03b): `billing_picking`, `billing_print` and the four action ticks — 5 users with canView+canEdit each; `place_order_ship_to` 5 view / 4 edit; `tint_panel_*` 3 (Chandresh #21, Deepanshu #25, Prakash #32); `reports_ti_report` those same 3 (view+export); `reports_tint_summary` 1 (Operations User #20).

**The helpers around the keys (all `lib/permissions.ts`):**
- **`REPORT_PAGE_KEYS`** (`:156-159`) = `reports_tint_summary`, `reports_ti_report`, and **`canViewAnyReport(allPerms)`** (`:165-167`): canView on ANY of them opens the `/reports` hub (`app/reports/page.tsx:42`) and shows the sidebar Reports row (`buildNavItems` `:193`). That row still carries PageKey **`ti_report`** as its identity only (`REPORTS_NAV_PAGE_KEY`, `:162`). **The `ti_report` tick gates nothing** since `6f628b05` (2026-09-17); it is labelled "Reports (legacy — no effect)" and kept for a later retirement.
- **`ACTION_PAGES` / `isActionAvailable()`** (`:406-483`): which non-View actions the code actually asks about per key, so `/admin/access` draws a checkbox or a dash (`app/(admin)/admin/access/page.tsx:129-133`). Advisory and cosmetic only — never used to filter what is stored. `canView` always returns true, so a key whose only meaning is canEdit still shows an inert View box. ⚠ `canExport` lists `mrn` and `reports_ti_report` only, while `app/api/ci/export/route.ts:45` checks `ci` canExport — so `/admin/access` draws a dash on CI's Export cell (see §13).
- **`PAGE_LABEL_OVERRIDES` / `pageLabel()`** (`:498-550`): display names for the 17 keys not in `PAGE_NAV_MAP` plus `ti_report` and the two `attendance` keys. The "Billing ·" prefix is what separates `billing_picking` from the floor board's `picking` on `/admin/access`.
- **`ACCESS_SECTIONS`** (`:558-584`): the 39 keys in 5 display groups — Operations, Tinting, Master data, Admin panel, Attendance — asserted against `ALL_PAGE_KEYS` by the access page.

`PAGE_NAV_MAP` labels `mail_orders` **"Billing"** (`:65`), and carries both `mrn` (`:85`) and `ci` (`:102`, since `55c3cdc6`, 2026-08-31) — each with a comment recording that its position was checked against `navItems[0]`.
...
| `ti_report` | tint_manager (view + export), admin — ⚠ the tick **gates nothing** since `6f628b05` (2026-09-17); the reports gate on `REPORT_PAGE_KEYS` (above) |
...
| `reports_tint_summary` · `reports_ti_report` | The per-report ticks that open `/reports` (any one of them) and each report. Seed (`prisma/seed.ts:273-276`): tint_manager + operation_manager view, plus export on `reports_ti_report` — seed, not live; live holders are listed above (Q03b). Was `ti_report` (reused) until 2026-09-17; now `REPORT_PAGE_KEYS` (`6f628b05`). Do not revert. |
...
| `mail_orders` | billing_operator (view + edit), operations (view + edit — **granted 2026-07-10**, one additive DB row, no code deploy), operation_manager (view + edit), tint_manager (**view + edit** — *corrected 2026-09-01: this row read "view only" from 2026-07-10; live SELECT shows `canEdit=true`*). Zero rows in `prisma/seed.ts` — DB-only, wiped on reseed. → `CLAUDE_MAIL_ORDERS.md §22`. Its sidebar label is **"Billing"** (`lib/permissions.ts:65`); the Billing desk itself → `CLAUDE_BILLING.md`. |
...
**`ROLE_HREF_OVERRIDES` — the same tick, three different URLs, and 11 of the 12 pages are the SAME COMPONENT** [2026-09-06]. `lib/permissions.ts:122-146`. `buildNavItems` decides *whether* a Master Data item appears from the person's tick, then rewrites the href by **primary job title**: `tint_manager` and `operation_manager` → `/tint/manager/*`, `dispatcher` → `/dispatcher/*`, everyone else → `/admin/*`. Twelve pages, four keys, three route groups. Verified file by file 2026-09-06:

| Page key | `/admin/*` | `/tint/manager/*` | `/dispatcher/*` |
|---|---|---|---|
| `customers` | `CustomersSplitView` | **`CustomersSplitView` — the same one** | `CustomersTable` |
| `skus` · `routes_areas` · `vehicles` | `SkusTable` · `RoutesTable` · `VehiclesTable` | identical per key | identical per key |

**So the three route groups are DUPLICATION, not variation.** The override decides which *copy* of an identical screen you land on, and the sole real difference in all twelve is `/dispatcher/customers` — a screen **nobody can reach** (see below). There is no "face" here to preserve: whenever the `/dispatcher` four are retired, this override list collapses to nothing for master data. ROADMAP step 7/8.

⚠ **A stale claim to correct where you find it, and it is NOT in this section.** The sentence *"`/admin/customers` uses the richer split view, the other three the same tables"* lives in **`lib/permissions.ts:127`** (a code comment above `ROLE_HREF_OVERRIDES`) and is quoted into **`ROADMAP.md` → Post-Support-retirement cleanup**. Both are describing the **retired Support-group** copies, so neither is wrong on its own terms — but read forward as a statement about the live tree it is misleading, because `/tint/manager/customers` renders `CustomersSplitView` too. The 2026-09-06 master-data gate report attributed the sentence to this §5; it was never here. Fix the comment when `lib/permissions.ts` is next edited — not worth its own commit (§3 forbids nothing here; it is simply not worth a deploy).

🔴 **The four `/dispatcher/*` master-data screens are reachable through the UI by NOBODY** [SELECT-verified 2026-09-06]. `app/(dispatcher)/layout.tsx` is five lines with no session read, so each page's own check is the only gate — and no sidebar anywhere generates those four hrefs, because **no dispatcher-role user holds any of the four ticks.** Three users carry the `dispatcher` role (Ajay Vansiya and Dhanraj Shah active, `Test Dispatcher` inactive) and all three hold zero. The three people who *do* hold a tick are sent elsewhere by their own primary role: Harsh (`admin`, no override) → `/admin/customers`; Chandresh and Prakash → `/tint/manager/customers`. By typing the URL: `/dispatcher/customers` admits Harsh, Chandresh and Prakash; the other three admit **Harsh alone**. **The route group is named for a role that cannot open it.** ⚠ This does not contradict §12's "these four are LIVE" — live and reachable are different claims, and §12 is answering "were they archived?" (no).
...
## 7. Database schema — v27.41

Versions: v21 base → v22 (mo_*) → v23 (orders dispatch) → v24 (customer match) → v25 (split) → v26 (mo_order_remarks) → v26.1 (isLocked) → v26.2 (mo_line_status) → v26.3 (carton + piecesPerCarton) → v26.4 (mo_learned_customers) → v26.5 (orders.orderDateTime) → v26.6 (user_roles + manual_tint_entries + users.phone + mo_sku_lookup.refDescription) → v27.0 (attendance foundation) → v27.1 (attendance settings hardening) → v27.2 (OT workflow + 2026-05-13 place-order v2 tables) → v27.3 (sampling_register + sampling_recipes + sampling_usage_log; orders.isRemoved + delivery_challans.isVoided; tint_skip_events + tint_pause_events; tint_assignments + import_raw_line_items netWeight/totalWeight) → v27.4 (sampling_usage_log.deliveryNumber backfill + tinter_issue_entries.samplingNo/shadeName) → v27.5 (customer_sales_officers + linkedSalesOfficerId on delivery_point_contacts + 3 columns on delivery_challan_formulas + sampling_recipes.packCode nullable with NULLS NOT DISTINCT + mo_sku_lookup_v2.isPrimary + mo_order_form_index_v2.mobileFamily) → v27.6 (mo_order_form_index_v2.region; Hide feature: `obd_visibility_rules` + `app_tag_settings` tables + orders.isHidden/hiddenById/hiddenReason/hiddenAt — §7.10) → v27.7 (Support gatekeeper + Hold/Dispatch-Target: orders.mailMatched; orders.heldAt, dispatchTargetDate, dispatchWindowId, arrivalSlotId; new `dispatch_slot_master` table — §7.4) → **v27.8** (Trip Report module, 2026-07-04/06: new standalone `trip_report` table — full columns → `CLAUDE_TRIP_REPORT.md §3`, §7.11 pointer here; `trip_report_delivery_no_dis_date_key` UNIQUE(deliveryNo, disDate); `mirror_trip_report_today` Postgres function) → **v27.9** (Support ship-to override, 2026-07-07: `orders.shipToOverrideCustomerId` Int? FK → `delivery_point_master`, relation `shipToOverrideCustomer` / `@relation("OrderShipToOverride")` — see dual-relation note in §7.3; `mo_orders.shipToOverrideCustomerId` Int? FK → `delivery_point_master`, relation `shipToOverrideCustomer` / `@relation("MoOrderShipToOverride")` — mo_orders' first relation to that table, no dual-relation trap) → **v27.10** (Picking Stage 2 — 2026-07-17/18 sessions, already shipped in code: `pick_assignments.checkedAt` DateTime? `@map("checked_at")` + `checkedById` Int? `@map("checked_by_id")`, relation `checkedBy` / `@relation("PickAssignmentCheckedBy")` — THIRD named relation from `pick_assignments` to `users`, alongside `picker`/`PickAssignmentPicker` and `assignedBy`/`PickAssignmentAssignedBy`, all correctly named on both sides today — no ambiguity. Supports the supervisor Approve step of the picking floor workflow — `CLAUDE_PICKING.md §6`) → **v27.11** (Flat SKU catalog, 2026-07-19, commit `916fcd39`: new standalone `sku_master_v2` table — 17 columns, FLAT, zero relations, `material` `@unique` as the natural key; mirrors `mo_sku_lookup_v2` MINUS `containerType`, PLUS `isActive` (new lifecycle flag) and `updatedAt` (`DateTime?`, hand-maintained, deliberately NO `@updatedAt`). Both timestamps carry `@db.Timestamptz(6)` — required, or Prisma emits plain `timestamp` and mismatches the live column. Built + poured via `docs/prompts/archive/2026-07/build-sku-master-v2-2026-07-19.sql`: 1,743 rows, 25 retired TOOLS `645xxxx` rows marked `isActive=false`. Old `sku_master` + its 3 FK helpers are now dead to operations, pending drop — §7.1.c) → **v27.12** (Push notifications + Picking live-sync, 2026-07-22: new standalone `push_subscriptions` table — 11 cols, `endpoint` UNIQUE `push_subscriptions_endpoint_key`, FK `userId`→users ON DELETE CASCADE, `updatedAt` a PLAIN `@default(now())` NOT `@updatedAt`; PLUS new index `orders_updatedAt_idx` on `orders("updatedAt" DESC)` backing the live-sync marker — §7.12) → **v27.13** (2026-07-20/30/31 additions, SELECT-verified live 2026-08-04; **minted 2026-08-05** in the reconciliation final pass): (a) `orders.pickEarlyReleasedAt` Timestamptz(6) + `pickEarlyReleasedById` FK→users `@relation("OrderPickEarlyReleasedBy")` (Picking early-release 5b, 2026-07-20 — `CLAUDE_PICKING.md`); (b) **Billing v2** (2026-07-30/31, `docs/prompts/archive/2026-08/billing-phase-2-build-decisions.md` — behaviour docs land in the MAIL_ORDERS session): new singleton `billing_settings` (scope UNIQUE `billing_settings_scope_key`, `rolloutStage` enforced by CHECK `chk_billing_settings_rollout_stage` OFF|TEST_USERS_ONLY|ALL_USERS — live row TEST_USERS_ONLY at mint; ALL_USERS since 2026-08-06, live 2026-09-18 Q01 — the flag is owned by `CLAUDE_BILLING.md §2`); `users.billingV2TestUser` Boolean (read fresh per page load by `lib/billing/flag.ts`, deliberately NOT JWT-cached — live: user 20 only); `orders.invoicedAt` Timestamptz(6) + `invoicedById` FK→users `@relation("OrderInvoicedBy")` (billing mark-done — 22 rows already carry it); partial index `orders_billing_pending_idx` ON orders("workflowStage") WHERE invoicedAt IS NULL AND invoiceNo IS NULL AND isRemoved=false (partial — NOT expressible in Prisma, do not model as `@@index`); `mo_orders.dispatchTargetDate` @db.Date + `dispatchWindowId` FK → `dispatch_slot_master` (slot INTENT — live FK name is all-lowercase `mo_orders_dispatchwindowid_fkey`, Postgres folded it; a camelCase FK search comes back empty and looks like a missing FK) + partial index `mo_orders_slot_intent_idx`; (c) `sku_master_v2.displayCategory` + `displayName` (deferred friendly-name columns — §7.1.c) → **v27.14** (2026-08-07): new standalone `pick_findings` table — 15 columns, the floor's "qty short / old stock" record against one raw line. FK to `orders`, a UNIQUE FK to `import_raw_line_items` (one finding per line), and **TWO named relations to `users`** (`PickFindingReportedBy` / `PickFindingRecordedBy` — both explicitly named on both sides, or Prisma errors). Live CHECK `chk_pick_findings_reason` and the partial `pick_findings_confirmed_idx` are NOT expressible in Prisma and are recorded by hand; `pick_findings_order_idx` IS modelled, with an explicit `map:`. Schema only at mint — **the UI landed the next day, see v27.15**. Full column list + landmines: §7.4. → **v27.15** (2026-08-08): `pick_findings.mfgMonth` INT? + `mfgYear` INT? — the OLD MFG half of `reason`, captured by the picker/supervisor popup. Live CHECK `chk_pick_findings_mfg_month` (month IS NULL OR 1-12) is NOT expressible in Prisma and is recorded by hand; there is deliberately NO year constraint. 🔴 The reason-dependency (REQUIRED on `old_mfg`, FORCED NULL on `short_quantity`) is **not enforced by the database** — it lives in both write routes and is written unconditionally on every save. §7.4. → **v27.16** (MRN module, 2026-08-20) and **v27.17** (2026-08-22: `mrn_line_batches.bestBeforeMonth`/`Year` made NULLABLE and retired from every surface) — ⚠ **both were minted in `prisma/schema.prisma` on 2026-08-20/22 but NEVER recorded in this chain**, which is why it read v27.15 until 2026-09-01 while the schema file already said v27.17. Bridged here rather than renumbered: the versions are real and shipped. Columns + landmines live in `prisma/schema.prisma`'s `mrn` / `mrn_lines` / `mrn_line_batches` headers. ✅ **The MRN module now has a canonical file — `docs/CLAUDE_MRN.md` (v1.0, 2026-09-01) — and it, not the 2026-08-20 design draft, is the authority.** That draft predates two days of change and is a historical record only. No §7 block is owed here: MRN's tables are documented in their own file. → **v27.18** (2026-09-01): new standalone `admin_audit_log` table — 9 columns, the WHO-changed-WHAT trail for admin writes that record no actor on the row itself. `id` **BigInt** (live BIGSERIAL, matching the `tint_skip_events`/`tint_pause_events` precedent), ONE FK `userId`→users (single FK, so the relation is deliberately UNNAMED — contrast the PickFinding*/Mrn* clusters in §7.3), `createdAt` Timestamptz(6). Three hand-named indexes, all modelled with an explicit `map:` because the live names are not what Prisma would generate: `admin_audit_log_created_idx` (createdAt DESC), `admin_audit_log_entity_idx` (entity, entityId), `admin_audit_log_user_idx` (userId). Written by exactly one caller, `lib/audit/log.ts`, which only ever INSERTs — append-only by convention, not by constraint. Wired at mint to 3 of the unattributed write paths (`admin/permissions` POST, `admin/users` POST, `admin/users/[id]` PATCH) — **that was the count on 2026-09-01 and it is history, not the current state: 43 routes are wired as of 2026-09-04.** Live coverage, and which routes are excluded or deferred and why, are in §7.13's coverage block; §13 carries the deferral. §7.13. → **v27.19** (2026-09-01, MRN photos + the OTR close — `CLAUDE_MRN.md §3-§4`): `mrn.closedAt` Timestamptz(6) + `closedById` Int? FK→users `@relation("MrnClosedBy")` — the FIFTH named relation from `mrn` to `users`, ON DELETE SET NULL like its three siblings and deliberately NOT the RESTRICT `createdById` carries; `chk_mrn_status` widened to a FOURTH value `closed`; new standalone `mrn_photos` table — 10 columns, FKs to `mrn` (CASCADE), `mrn_lines` (CASCADE) and `users` (RESTRICT, `@relation("MrnPhotoCapturedBy")`), `storagePath` UNIQUE, two live CHECKs Prisma cannot see (`chk_mrn_photo_kind`, `chk_mrn_photo_lr_truck_level`). Objects live in a PRIVATE Supabase bucket `mrn-photos` created by hand — not by SQL, and deliberately NOT a prefix inside `attendance-photos`, whose purge cron would reach them. → **v27.20** (2026-09-01, the delivery split — `CLAUDE_MRN.md §5`): `mrn_lines.deliveryNo` text NOT NULL — one STI can carry several delivery numbers, so the number moved off the header onto the LINES; the unique index moved with it, `(mrnId, lineNo)` → `(mrnId, deliveryNo, lineNo)`, plus `mrn_lines_mrnId_deliveryNo_idx`. ⚠ The column still carries a TEMPORARY `DEFAULT ''` — scaffolding so it could be added NOT NULL to a populated table, dropped by the still-commented Part 4 of `docs/prompts/drafts/sql-2026-09-01-mrn-delivery-split.sql`. `mrn.deliveryNo` is KEPT but FROZEN: no writer since 2026-09-01, real history for the MRNs raised before it. `mrn.otrNo` likewise has exactly one writer now, `POST /api/mrn/[mrnId]/close`. ⚠ **Both of these were recorded in the chain ON THE DAY, which the v27.16/17 entry above exists to explain was not done last time.** → **v27.21** (CI module — Goods Return Note, **2026-08-31**, three tables live in one SQL pass by `sql/2026-08-31-ci-module.sql`; `ci_returns.source` + its CHECK followed on 2026-09-01 with the auto-CI, `618f67fc`): new standalone `ci_reason_master` (7 cols, `code` UNIQUE, 8 seeded rows — DATA not code, the depot relabels a reason without a deploy), `ci_returns` (**32 cols** — 31 until `source`; the `schema.prisma` header said 31 until it was corrected 2026-09-03) and `ci_return_lines` (12 cols, `UNIQUE(ciReturnId, lineNumber)`, and `rawLineItemId` is a plain nullable Int **pointer, NOT an FK** — contrast `pick_findings`, which does carry a UNIQUE FK to that table). `ci_returns` carries **FIVE named relations**: three to `users` (`CiReturnSupervisor` / `CiReturnBillingOperator` / `CiReturnVoidedBy` — all three explicitly named on both sides, or Prisma errors at generate time, §7.3's rule), plus `CiReturnOrder` → `orders` (ON DELETE Restrict) and `CiReturnCustomer` → `delivery_point_master` (SET NULL; named on one FK only, deliberately, to match the name the other side carries). **FIVE live CHECK constraints, none expressible in Prisma** (§7.4) and all on `ci_returns`: `chk_ci_returns_status` (draft|submitted|closed|returned_to_floor — the fourth is allowed from day one and written by nothing), `_return_type` (full|part), `_material_moved` (moved|not_moved), `_source` (manual|auto_finding), and the load-bearing `chk_ci_returns_complete_when_not_draft`, which is what makes four NULLABLE columns safe: a draft may carry NULL, anything that is not a draft may not. Six indexes on `ci_returns` (pkey, `ciNumber` UNIQUE, + `invoiceNo` / `obdNumber` / `orderId` / `status` / `materialReceivedDate`); **`orders_invoiceNo_idx` was added in the same 2026-08-31 pass** and had also never been recorded here — half of every CI search was a sequential scan before it. ⚠ **MINTED LATE, ON 2026-09-03, AND THIS ENTRY IS A RECORD RATHER THAN A PATCH.** The tables went live on 2026-08-31 — *before* v27.19 and v27.20 — and were never entered in this chain at all: a `grep ci_returns` over this file returned zero hits for three days while the module shipped nineteen commits and reached production. **This is the SECOND time**, the first being v27.16/17 (MRN), which the entry two steps above exists to explain. The pattern is the same both times: a module builds fast, the tables land in one SQL pass, and the chain — which nothing enforces and no build step checks — is the one artefact nobody circles back to. Recording it silently would have taught nothing. ✅ **No §7 block is owed**, exactly as v27.20 ruled for MRN: the columns, their nullability and the reasoning all live in `prisma/schema.prisma`'s three model headers and in **`docs/CLAUDE_CI.md` (v1.0, 2026-09-03), which is the authority** — not the 2026-08-31 design draft, which predates three days of change and is wrong in at least one countable way (it describes nine API routes; there are twelve). → **v27.22** (per-user access, 2026-09-04: new standalone `user_page_access` — 10 columns, one row per (user, pageKey), `UNIQUE(userId, pageKey)` as the ON CONFLICT target for the save route plus a `pageKey` index for the reverse "who can reach Floor?" lookup. ONE FK to `users` (CASCADE), so the relation is deliberately UNNAMED. Filled by `sql/2026-09-04-user-page-access.sql` with **1,053 rows = 39 users × 27 ALL_PAGE_KEYS**, reproducing `getAllPermissionsForRoles()` exactly — admin short-circuit, OR-merge across `user_roles`, and `lib/auth.ts:221`'s rule (then :208) that any `user_roles` rows REPLACE the primary rather than adding to it — then parity-checked against the live role system at **0 diffs**, which is why the cutover changed nobody's access. §7.14) → **v27.23** (superuser, 2026-09-04: `users.isSuperuser` Boolean NOT NULL DEFAULT false — the `admin` JOB TITLE becomes a flag on the person, checked by 68 gates as **flag OR role**, never the flag alone. Live: 1 of 39 users. Rides the JWT as an OPTIONAL claim, re-read on the existing 5-minute attendance stale window, so a grant or revoke lands without a sign-out. §7.15, and the two-arm rule in §13.) → **v27.24** (trip mirror — hash-gated upsert, **2026-09-08**, applied in the Supabase SQL Editor and verified the same day; plan of record `docs/prompts/archive/2026-09/code-resume-2026-09-08-trip-mirror-rewrite.md`, evidence `docs/prompts/archive/2026-09/code-discovery-2026-09-08-trip-mirror.md`): `trip_report."rowHash"` text NULLABLE — md5 of the incoming JSON row **minus `sourceId`** — plus a new standalone `mirror_heartbeat` table (7 cols: `id` text PK seeded with one row `'trip_report'`, `lastRunAt` / `lastChangedAt` Timestamptz(6), `rowsSeen` / `rowsChanged` / `rowsRemoved` / `rowsSkipped` Int; camelCase, NO `@map`, zero relations, no RLS — matching `trip_report`), and a new function `trip_mirror_ping(p_rows integer) returns void` which stamps liveness and does no mirror work. **`mirror_trip_report_today(rows jsonb) returns integer` was REPLACED, not dropped** (same signature, `CREATE OR REPLACE` — the puller fires every ~62s and a DROP/CREATE window is not theoretical): the `CREATE TEMPORARY TABLE _incoming` is gone in favour of CTEs, and the delete-and-refill is now a **DELETE of only the rows absent from the batch, scoped to EVERY distinct `disDate` in the batch, followed by an `ON CONFLICT ("deliveryNo","disDate") DO UPDATE … WHERE trip_report."rowHash" IS DISTINCT FROM EXCLUDED."rowHash"`**. ⚠ **`sourceId` is excluded from the hash and that is load-bearing.** The reason recorded until now — "NTS reissues it on every pull", `CLAUDE_TRIP_REPORT.md §2/§3` — is **FALSE, measured 2026-09-08**: 74 of 74 `sourceId`s were identical across two consecutive live cycles, and 6,779 rows carry 6,779 distinct ids ascending by date with no cross-day overlap. That matters twice over: had ids really churned, an unchanged (therefore unwritten, therefore frozen) row could collide with a new row on `trip_report_pkey` and the upsert design would be unsafe; and excluding it is still right, because `sourceId` is functionally dependent on `(deliveryNo, disDate)` and adds nothing to a content hash. **Why:** the old body rewrote every row of the day every ~62s, 24/7 — 13,555 inserts / 13,438 deletes / **0 updates** on a ~100-row table — and created and dropped a 37-column temporary table ~1,390 times a day; steady state now writes nothing (`rowsChanged = 0`, and `n_tup_upd` moved off 0 for the first time). **Two unguarded edges in the old body were closed in the same pass:** a batch spanning IST midnight returned two dates while `target_date := max("disDate")` cleared only the newer one and the INSERT was unconditional, so the older date collided with `trip_report_delivery_no_dis_date_key` and failed the WHOLE cycle; and a row whose `disDate` parsed to NULL was insertable but never deletable (equality never matches NULL) — such rows, and rows with a NULL/blank `deliveryNo` which the *upsert* newly makes dangerous, are now skipped and counted into `rowsSkipped`. **The return value is deliberately the DEDUPED ROW COUNT, not the changed count**, because the PowerShell puller is UNCHANGED and logs `mirrored N rows for today`: returning "changed" would print `mirrored 0 rows` on every quiet cycle, which is exactly what a real failure prints. ✅ **No §7 block is owed** — same ruling as v27.20 (MRN) and v27.21 (CI): the columns and the reasoning live in `prisma/schema.prisma`'s `MirrorHeartbeat` header and in `CLAUDE_TRIP_REPORT.md`, whose own §2/§3/§7 corrections are a **separate, still-owed job** (ten of them, listed in §8 of the resume draft — the puller's real path, the undocumented `modi_inv='FRT'` filter, the real 61–65s cadence, the two live index names, and the `sourceId` falsehood above). ⚠ **The PowerShell side is PHASE B and is BLOCKED**: the 2026-09-08 discovery proved the live puller runs on an **unidentified host** — not this machine, which has no such process, no scheduled task and a `log.txt` seven days stale while production took a mirror write every 62 seconds. Phase A was built to need none of it; `trip_mirror_ping` sits ready and uncalled until that host is named. ✅ **Recorded in this chain ON THE DAY** — which is what the v27.16/17 and v27.21 entries above exist to say was not done the previous two times.)

**Chain continued — v27.25 … v27.35, minted 2026-09-19 in the canon sweep, from `git show <hash> -- prisma/schema.prisma` plus the committed SQL, each checked against the live read of 2026-09-18** (`docs/prompts/drafts/sql-2026-09-18-canon-sweep-live-results.csv`, Q-numbers below). One commit = one version, in commit order. ⚠ **v27.25–v27.27 shipped in AUGUST, before v27.24 by date, and were never numbered at the time; they are numbered retroactively in commit order, so chain order ≠ calendar order for those three.** 🔴 This is the **third** time a module's schema reached production outside this chain (MRN v27.16/17, CI v27.21, now the trip module and its neighbours). Constraint and index text below is copied from the live read; where the live read has no row for an object, the source is named and it says "not in the live read".

- **v27.25** · 2026-08-07 · `6409f07c` · `users.notesFontSize` integer NOT NULL DEFAULT 11 — the per-user notes-band text size on `/mail-orders` (behaviour: `CLAUDE_BILLING.md §9`). Its CHECK `chk_users_notes_font_size` was 11–15 at `6409f07c` and was widened to **11–20** the same day by `2916b5f8`; both bounds come from the schema comment (`prisma/schema.prisma:153-155`) and `lib/mail-orders/notes-font-size.ts:26-27` — **not in the live read**. · live-verified 2026-09-18 (Q10a: `integer | nullable=NO | default=11`).
- **v27.26** · 2026-08-10 · `71e7b53a` · `mo_orders.updatedAt` timestamptz NOT NULL DEFAULT now(), **owned by a database trigger**: `trg_mo_orders_updated_at` `BEFORE UPDATE → EXECUTE FUNCTION set_updated_at_mo_orders()`. Modelled as `@default(now())` and deliberately **NOT** `@updatedAt` — the trigger is the single writer (the opposite case to `push_subscriptions`, §13). · live-verified 2026-09-18 (Q10a: `timestamp with time zone | nullable=NO | default=now()`; Q11: 1 trigger row).
- **v27.27** · 2026-08-21 · `f7c8d232` · `route_master.bayNumber` integer NULL — the loading bay a route is stacked at. Not unique and not a route id: several routes share a bay. Read by `lib/picking/queue.ts:310` and `lib/floor/queries.ts:366`. · live-verified 2026-09-18 (Q10a: `integer | nullable=YES | default=none`).
- **v27.28** · 2026-09-09 · `16eff3d5` · new table **`app_settings`** (5 columns: `id`, `settingKey` UNIQUE, `isEnabled` default false, `updatedById` → users `@relation("AppSettingsUpdatedBy")`, `updatedAt` `@default(now()) @updatedAt`) — per-FLAG, default-OFF, not to be confused with per-badge `app_tag_settings` (§7.10). Its unique and FK (`app_settings_settingKey_key`, `app_settings_updatedById_fkey`) come from the schema comment — **not in the live read**. Plus `orders.pickVisibleAt` timestamptz NULL and `orders.pickVisibleById` int NULL → users `@relation("OrderPickVisibleBy")`. 🔴 **`pickVisibleAt/ById` are PRESENT, DELIBERATELY UNUSED: no reader or writer** in `app/`, `lib/` or `components/` since slice 8 (`791a2cd6`) moved the gate to `trips.shownAt` — every hit is a comment (grep, both ways, 2026-09-19); only two underscore scratch scripts outside the `tsc` gate name them. · live-verified 2026-09-18 (Q10b: `app_settings` exists; Q10a: both columns `nullable=YES`; Q04: one row, `picking.visibilityGate` `isEnabled=false`). Schema block §7.16.
- **v27.29** · 2026-09-09 · `aead3c32` · new tables **`trips`** (24 columns at mint) and **`trip_drops`** (11 columns), with seven CHECKs on `trips` present from this mirror — `chk_trips_status` (then including `'loading'`, see v27.31), `chk_trips_type_code`, `chk_trips_seq_positive`, `chk_trips_vehicle_one_of`, `chk_trips_dispatched_complete`, `chk_trips_cancelled_complete`, `chk_trips_number_shape` (rewritten in v27.33) — and two on `trip_drops` (`chk_trip_drops_key`, `chk_trip_drops_seq_positive`); `trips_tripNumber_key` UNIQUE; the full UNIQUE `trips_date_type_seq_key` (replaced in v27.33); indexes `trips_date_idx`, `trips_status_idx`, `trips_vehicle_idx`, `trip_drops_trip_idx`, `trip_drops_customer_idx`; uniques `trip_drops_trip_key_key`, `trip_drops_trip_seq_key`. Definitions verbatim in §7.16. ⚠ The mirror's comment said `chk_trips_type_code` admits `('L','U','I')`; `62ea5f1b` recorded that the live CHECK admitted `'C'` all along. Plus `orders.tripDropId` int NULL → `trip_drops` (`onDelete: SetNull` and `orders_tripDropId_idx` per `schema.prisma` — the FK and index are **not in the live read**); `orders.loadedAt` timestamptz NULL / `orders.loadedById` int NULL → users `@relation("OrderLoadedBy", onDelete: SetNull)` — 🔴 **PRESENT, DELIBERATELY UNUSED: no reader or writer anywhere** (grep, both ways, 2026-09-19: zero hits outside `prisma/schema.prisma`); `transporter_master.isRealTransporter` boolean NOT NULL DEFAULT false (filters `app/api/floor/trips/options/route.ts:77`). · live-verified 2026-09-18 (Q05a, Q05b, Q10a, Q10b).
- **v27.30** · 2026-09-11 · `abd495f4` (SQL `sql/2026-09-11-hide-tag-scope.sql`, committed in `fb2f3853`) · `app_tag_settings` gains `scope` text NOT NULL DEFAULT 'everyone', `roleSlug` text NULL, `userId` int NULL (FK users ON DELETE CASCADE); `chk_app_tag_settings_scope`; three partial unique indexes `app_tag_settings_everyone_key` / `_role_key` / `_user_key` and two partial plain indexes `_role_idx` / `_user_idx`; the single-column UNIQUE on `tagKey` **DROPPED**. `upsert` on `tagKey` is no longer possible. Definitions verbatim in §7.10. · live-verified 2026-09-18 (Q05a, Q05b, Q10a).
- **v27.31** · 2026-09-14 · `c539aae4` · `chk_trips_status` drops `'loading'` (never written; zero rows) → `draft | released | dispatched | cancelled`. No SQL file committed; the `schema.prisma` comment is the mirror. · live-verified 2026-09-18 (Q05a).
- **v27.32** · 2026-09-14 · `c84f8faa` (SQL `db/slice-2-trip-activity.sql`) · new table **`trip_activity`** (7 columns) with `trip_activity_trip_fkey` ON DELETE RESTRICT, `trip_activity_actor_fkey`, `chk_trip_activity_action` (8 values at creation), indexes `trip_activity_trip_idx` and `trip_activity_created_idx`. · live-verified 2026-09-18 (Q05a, Q05b).
- **v27.33** · 2026-09-15 · `62ea5f1b` (SQL `sql/2026-09-15-trip-number-reuse.sql`) · the full UNIQUE `trips_date_type_seq_key` is **replaced** by the partial unique index `trips_date_type_seq_live_key … WHERE (status <> 'cancelled'::text)`; `chk_trips_number_shape` rewritten so a cancelled trip carries `-C` / `-C2` … and `lpad` pads to `greatest(2, length(seq))`; `chk_trip_activity_action` gains `'renamed'`. · live-verified 2026-09-18 (Q05a, Q05b).
- **v27.34** · 2026-09-15 · `791a2cd6` (SQL `sql/2026-09-15-slice8-show-per-trip.sql`) · `trips.shownAt` timestamptz(6) NULL, `trips.shownById` int NULL + `trips_shownById_fkey` ON DELETE SET NULL; `chk_trip_activity_action` gains `'shown'`, `'taken_back'`. The same SQL cleared every stale `orders.pickVisibleAt/ById` value (data, not schema). · live-verified 2026-09-18 (Q05a).
- **v27.35** · 2026-09-15 · `22ced2d8` (SQL `sql/2026-09-15-slice9-print-tab.sql`) · `trips.sentToBillingAt` / `sentToBillingById` / `billingCopiedAt` / `billingCopiedById` + `trips_sentToBillingById_fkey` and `trips_billingCopiedById_fkey`, both ON DELETE SET NULL; `chk_trip_activity_action` gains `'sent_to_billing'`, `'taken_back_from_billing'`, `'invoices_copied'` (14 values in all). The same SQL granted `billing_print` in `role_permissions` and `user_page_access` by copying `billing_picking` (data). · live-verified 2026-09-18 (Q05a).
- **v27.36** · 2026-09-19 · `f8647a6b` (SQL `sql/2026-09-19-route-clubs.sql`) · new tables **`route_clubs`** (5 columns) and **`route_club_members`** (5 columns) — routes that share a truck, one card on Floor's By route view. `route_clubs_delivery_type_fkey` ON DELETE RESTRICT; `route_club_members_club_fkey` is a **composite FK** `("clubId","deliveryTypeId") → route_clubs(id,"deliveryTypeId") ON DELETE CASCADE`; `route_club_members_route_fkey` ON DELETE RESTRICT; uniques `route_clubs_id_type_key`, `route_clubs_type_name_key`, `route_clubs_type_sort_key`, `route_club_members_type_route_key`, `route_club_members_club_sort_key`; CHECKs `chk_route_clubs_sort`, `chk_route_club_members_sort`. Data: the three Local clubs, `sql/2026-09-19-route-clubs-local.sql` (`ac493149`). Definitions and rules in §7.17. · live-verified 2026-09-19 (the file's own check: 10 columns + 10 constraints; the seed's check: 6 member rows). ✅ **Recorded in this chain ON THE DAY.**
- **v27.37** · 2026-09-19 · `a034709e` (Prisma model) · SQL `sql/2026-09-19-load-plan-config.sql` (`ce10bfb1`) · new table **`load_plan_config`** (3 columns: `deliveryTypeId` integer PK and FK → `delivery_type_master` ON DELETE RESTRICT (`load_plan_config_delivery_type_fkey`), `config` jsonb NOT NULL, `updatedAt` timestamptz(6) NOT NULL DEFAULT now()) + CHECK `chk_load_plan_config_object` (`jsonb_typeof(config) = 'object'`), seeded with the Upcountry rules in the same DO block. §7.18. ⚠ **NOT YET RUN LIVE when this entry was written** — the owner runs the file in the SQL Editor; until then the app reads "Load plan not set up" (the reader never throws). Recorded on the day so the chain is not behind the code a fourth time.
- **v27.38** · 2026-09-21 · SQL `sql/2026-09-21-load-plan-v2.sql` (generated from `docs/data/load-plan/*.csv`; 🔴 the SQL and the rates CSV are **NOT in git** — they carry rates and the repo is public; see §7.19) · new tables **`load_plan_area_rate`** (11 columns) and **`load_plan_area_pair`** (3 columns), and eight NEW KEYS on the Upcountry `load_plan_config.config` (existing keys kept): `routeSides`, `vehicles`, `maxPlacesPerTruck`, `pairMinTimes`, `bulkKg`, `truckPenaltyRs`, `holdSmallUnlessOverdue`, `newArea`. One DO block, all or nothing. §7.19. ⚠ **NOT YET RUN LIVE when this entry was written**; no Prisma model yet (nothing reads the tables — the engine step adds the models). 🔴 **Rates are internal — never sent to a browser** (§7.19).
- **v27.39** · 2026-09-22 · SQL `sql/2026-09-22-billing-telephonic.sql` (Prisma models in the same commit) · **RUN LIVE 2026-09-22 and verified** — text below is the live `pg_constraint` / `pg_indexes` read, not the code. New table **`so_tags`** (13 columns; SO numbers billing tags `hold` / `ci` for telephonic orders): `so_tags_pkey`; `chk_so_tags_tag` `CHECK ((tag = ANY (ARRAY['hold'::text, 'ci'::text])))`; `chk_so_tags_status` `CHECK ((status = ANY (ARRAY['waiting'::text, 'matched'::text])))` — "expired" is derived at read time, never stored; `so_tags_addedById_fkey` `FOREIGN KEY ("addedById") REFERENCES users(id) ON DELETE RESTRICT`; `so_tags_removedById_fkey` `… ON DELETE SET NULL`; 🔴 the **partial** unique index `so_tags_soNumber_live_key` `UNIQUE … USING btree ("soNumber") WHERE ("isRemoved" = false)` — not expressible in Prisma, recorded as a model comment (same class as `trips_date_type_seq_live_key`), so there is **no `findUnique` by `soNumber`**. New table **`so_tag_matches`** (6 columns; one row per OBD a tag hit, `ciSkipReason` nullable): `so_tag_matches_pkey`; `so_tag_matches_soTagId_orderId_key` `UNIQUE ("soTagId", "orderId")`; `so_tag_matches_soTagId_fkey` → `so_tags(id)` and `so_tag_matches_orderId_fkey` → `orders(id)`, both `ON DELETE RESTRICT`; index `so_tag_matches_orderId_idx` `USING btree ("orderId")` (the FK's referencing side). Every name is Prisma's default, so no `map:`. Two FKs to `users` → relations **`SoTagAddedBy`** / **`SoTagRemovedBy`**, named on both sides (§7.3); `SoTagMatchTag`, `SoTagMatchOrder`. **`chk_ci_returns_source` widened** — was `CHECK ((source = ANY (ARRAY['manual'::text, 'auto_finding'::text])))`, now `CHECK ((source = ANY (ARRAY['manual'::text, 'auto_finding'::text, 'auto_bill_only'::text])))`; nothing writes `auto_bill_only` yet (`CLAUDE_CI.md §3`). Data: an all-false `user_page_access` row for page key **`billing_telephonic`** for every user — 40 rows, 0 granted. ⚠ The key is not yet in `ALL_PAGE_KEYS` (the page is not built); the rows are ahead of the code on purpose, like the 2026-09-11 fill.
- **v27.40** · 2026-09-22 · SQL `sql/2026-09-22-ci-source-floor.sql` (a RECORD — run live 2026-09-22 by Smart Flow before the file was written) · **`chk_ci_returns_source` widened to a fourth value** — was `CHECK ((source = ANY (ARRAY['manual'::text, 'auto_finding'::text, 'auto_bill_only'::text])))`, now `CHECK ((source = ANY (ARRAY['manual'::text, 'auto_finding'::text, 'auto_bill_only'::text, 'floor'::text])))`. `'floor'` = a full-bill, material-not-moved CI raised from the Floor desk, which also takes the bill off the floor; writer `app/api/floor/ci/route.ts`. No Prisma model change (`source` is a plain String); the code mirror is `CiSource` / `ciSourceTag` in `lib/ci/types.ts` and the `ci_returns` comment block in `prisma/schema.prisma`. ✅ Recorded in this chain on the day.
- **v27.41** · 2026-09-24 · SQL `sql/2026-09-24-billing-mo-actions.sql` (Prisma models in the same commit) · **RUN LIVE 2026-09-24 and verified** by the file's own PART 6 (8 columns present, 3 FKs `ON DELETE SET NULL`, the index present, 0 rows marked) · Billing mail-order actions, design `docs/prompts/drafts/web-update-2026-09-24-billing-mo-actions.md` (v4). **`mo_orders`** gains `billOnlyAt` timestamptz NULL + `billOnlyById` int NULL → users (`mo_orders_billOnlyById_fkey`, SET NULL, relation **`MoOrderBillOnlyBy`**) — billing pressed **CI** (bill-only) — and `handAt` timestamptz NULL + `handById` int NULL → users (`mo_orders_handById_fkey`, SET NULL, **`MoOrderHandBy`**) — billing pressed **Hand** (the dealer collects). `mo_orders` now has THREE FKs to `users` (with `MoOrderPunchedBy`), all named on both sides (§7.3). **`orders`** gains `handAt` / `handById` → users (`orders_handById_fkey`, SET NULL, **`OrderHandBy`** — the EIGHTH named users relation on `orders`). 🔴 Hand is a timestamp + actor, **never a `dispatchStatus` value** (every board predicate pins `'dispatch'`). **`so_tags`** gains `fromMailOrder` boolean NOT NULL DEFAULT false — the tag was written because a mail order is marked CI, not typed on the Telephonic tab; a FLAG, not a `moOrderId`, because one live tag per SO can be shared by two marked mail orders. **`trips`** gains `isHand` boolean NOT NULL DEFAULT false — a Hand trip (no vehicle; stays `draft` for life). 🔴 One **partial** index, not expressible in Prisma and recorded as a model comment: `mo_orders_billOnly_soNumber_idx ON mo_orders ("soNumber") WHERE "billOnlyAt" IS NOT NULL` (same class as `so_tags_soNumber_live_key`). Every FK name is Prisma's default, so no `map:`. No data written, no backfill (NULL/false = "not marked"). The typed ship-to + challan part of the design (§5) is **parked** and none of it is in this version. ✅ Recorded in this chain on the day.

⚠ **Known gap:** `sql/2026-09-21-trips-vehicle-size.sql` and `sql/2026-09-21-load-plan-snapshot.sql` ran live but were never minted — to be numbered in the next consolidation. (Both files exist in the repo, committed in `7aa24160`; `trips.vehicleSize` is already in `prisma/schema.prisma`.)

Commits in the same window that touched `prisma/schema.prisma` and minted nothing, because they change comments only: `2916b5f8` (the notes-size bound, folded into v27.25), `36103761`, `8eaa4663`, `cdbf95b1`. `sql/2026-09-15-slice6-settle-drafts.sql` is a data migration (draft trips with a vehicle → `released`), not a schema change.
...
role_master                Roles — ids 1-7 + 12-17, 13 rows live (id 17 = floor_access; see §5;
                           live 2026-09-18 Q07a)
...
users                      Depot staff. bcryptjs 10 rounds. roleId FK. phone TEXT (nullable, 10-digit).
                           ...
                           notesFontSize INT NOT NULL DEFAULT 11 (v27.25) — CHECK 11-20 by
                           schema comment; read fresh per page load, not on the JWT
                           (CLAUDE_BILLING.md §9).
...
transporter_master, vehicle_master
                           transporter_master.isRealTransporter BOOLEAN NOT NULL DEFAULT false
                           (v27.29) — true = a real carrier offered in the trip picker; NOT
                           the same question as isActive (CLAUDE_FLOOR_TRIPS.md §3.5).
...
route_master, area_master, area_route_map, sub_area_master
                           route_master.bayNumber INT? (v27.27) — loading bay; shared by
                           several routes, NOT unique, NOT a route id.
...
orders                     workflowStage, slotId, originalSlotId, dispatchSlotDeadline,
                           ...
                           write (app/api/import/obd/route.ts:263) — and no reader. Kept: it is
                           ...
                           PICK-VISIBLE columns (2026-09-09, v27.28 — §7):
                           pickVisibleAt TIMESTAMPTZ?, pickVisibleById INT? FK → users
                           relation @relation("OrderPickVisibleBy"). 🔴 PRESENT, DELIBERATELY
                           UNUSED: no reader or writer since slice 8 (791a2cd6) moved the pick
                           gate to trips.shownAt (§7.16). Do not read meaning into them.

                           TRIP pointer (2026-09-09, v27.29 — §7.16):
                           tripDropId INT? FK → trip_drops.id, onDelete SetNull, indexed
                           (orders_tripDropId_idx). THE one trip pointer — there is no
                           orders.tripId; reach the trip via trip_drops.tripId. NULL = on no
                           trip, the normal state. Behaviour → CLAUDE_FLOOR_TRIPS.md §3.5/§7.

                           LOADED columns (2026-09-09, v27.29 — §7):
                           loadedAt TIMESTAMPTZ?, loadedById INT? FK → users
                           relation @relation("OrderLoadedBy"), onDelete SetNull.
                           🔴 PRESENT, DELIBERATELY UNUSED: no reader or writer anywhere.

                           HAND columns (2026-09-24, v27.41 — §7):
                           handAt TIMESTAMPTZ?, handById INT? FK → users, ON DELETE SET NULL,
                           relation @relation("OrderHandBy"). Billing's "Hand" mark — the dealer
                           collects from the depot. Carried from mo_orders.handAt/ById by
                           enrichment, dual-written by the billing actions route. 🔴 NEVER a
                           dispatchStatus value (every board predicate pins 'dispatch').

                           ⚠ Every FK to users on this table MUST carry a named @relation on both
                           sides — EIGHT exist now (OrderPickedBy / OrderRemovedBy / OrderRestoredBy /
                           OrderPickEarlyReleasedBy / OrderInvoicedBy / OrderPickVisibleBy /
                           OrderLoadedBy / OrderHandBy; hiddenById is a bare Int with no relation) —
                           an unnamed one is a Prisma ambiguity ERROR.
...
- **Status [LIVE].** `evaluateDispatchSlot()` (`lib/dispatch/dispatch-engine.ts`) is imported
  ...
  dispatch **date + window** at enrichment, per-`soNumber` (never a full-table scan). It has two more
  call sites in the same file: `applyNoMailOrderFallback` (`:599`, the import auto-release —
  `CLAUDE_IMPORT.md §2.1`) and the patch-headers dispatch-window repair (`:4334`,
  `CLAUDE_IMPORT.md §12.1b`). Tint completion reaches it through `lib/dispatch/completion-slot.ts`
  (`CLAUDE_TINT.md §2`). Pure function,
...
- **What it writes** (`route.ts:416-424`): `dispatchTargetDate`, `dispatchWindowId`,
  `dispatchSlotSource='auto'`, `dispatchSlotRuleId` (§7.3).
- **Manual-skip guard** (`route.ts:381`): it SKIPS any order already `dispatchSlotSource='manual'` —
...
- **Live recount (SELECT 2026-07-27): 1,546 at `dispatched`** — roughly **500 rows moved in three days**,
  ...
  2026-07-24 line above stays as the dated snapshot it was. Live 2026-09-18: **7,330** at
  `dispatched` (Q09); every writer of that stage is listed in `CLAUDE_FLOOR_TRIPS.md §14`.
- **Parked (not the engine's fault):** the `Deco` (9 rows) un-mapped SMU never matches `Deco Retail`
  so never auto-slots. A bill with no mail order no longer waits at `pending_support` with
  `dispatchStatus` NULL: since `b3dfe5b8` (2026-09-11) `applyNoMailOrderFallback` releases every
  such non-tint bill at import with `dispatchStatus='dispatch'` and an engine slot where the engine
  gives one — owner `CLAUDE_IMPORT.md §2.1`, not re-described here.
...
mo_orders                  Per parsed email
                           ...
                           As-built record: docs/prompts/archive/2026-08/billing-phase-2-build-decisions.md.
                           updatedAt TIMESTAMPTZ NOT NULL DEFAULT now() (v27.26) — kept current
                           by the DB trigger trg_mo_orders_updated_at (BEFORE UPDATE →
                           set_updated_at_mo_orders()). Prisma: @default(now()), deliberately
                           NOT @updatedAt, and no code writes it — §13.
                           BILLING MARKS (2026-09-24, v27.41 — §7): billOnlyAt TIMESTAMPTZ? +
                           billOnlyById INT? FK → users, SET NULL, @relation("MoOrderBillOnlyBy")
                           — billing pressed CI (bill-only; the CI is raised at import through an
                           so_tags row with fromMailOrder = true). handAt TIMESTAMPTZ? + handById
                           INT? FK → users, SET NULL, @relation("MoOrderHandBy") — billing pressed
                           Hand; carried onto orders.handAt/ById by enrichment. mo_orders now has
                           THREE FKs to users (+ MoOrderPunchedBy) — all named on both sides.
                           🔴 Partial index mo_orders_billOnly_soNumber_idx ON ("soNumber")
                           WHERE "billOnlyAt" IS NOT NULL — NOT expressible in Prisma, recorded
                           as a model comment; do not model it as @@index.
...
app_tag_settings           Per-badge on/off, SCOPED (v27.30, 2026-09-11,
                           sql/2026-09-11-hide-tag-scope.sql). id, tagKey TEXT (NOT unique),
                           scope TEXT NOT NULL DEFAULT 'everyone', roleSlug TEXT?, userId INT?
                           (live FK → users ON DELETE CASCADE; a bare scalar in Prisma),
                           isEnabled BOOLEAN DEFAULT true, updatedById, updatedAt.
                           Default-ON (no row = badge shows). One row per (tagKey, audience).
                           Live (2026-09-18, Q05a/Q05b), verbatim, one per line:
chk_app_tag_settings_scope: CHECK ((((scope = 'everyone'::text) AND ("roleSlug" IS NULL) AND ("userId" IS NULL)) OR ((scope = 'role'::text) AND ("roleSlug" IS NOT NULL) AND ("userId" IS NULL)) OR ((scope = 'user'::text) AND ("roleSlug" IS NULL) AND ("userId" IS NOT NULL))))
app_tag_settings_userId_fkey: FOREIGN KEY ("userId") REFERENCES users(id) ON DELETE CASCADE
CREATE UNIQUE INDEX app_tag_settings_everyone_key ON public.app_tag_settings USING btree ("tagKey") WHERE (scope = 'everyone'::text)
CREATE UNIQUE INDEX app_tag_settings_role_key ON public.app_tag_settings USING btree ("tagKey", "roleSlug") WHERE (scope = 'role'::text)
CREATE UNIQUE INDEX app_tag_settings_user_key ON public.app_tag_settings USING btree ("tagKey", "userId") WHERE (scope = 'user'::text)
CREATE INDEX app_tag_settings_role_idx ON public.app_tag_settings USING btree ("roleSlug") WHERE ("roleSlug" IS NOT NULL)
CREATE INDEX app_tag_settings_user_idx ON public.app_tag_settings USING btree ("userId") WHERE ("userId" IS NOT NULL)
                           🔴 The single-column UNIQUE on tagKey was DROPPED. Prisma cannot model
                           a partial index, so `upsert` is unavailable: writes go findFirst →
                           update / create / delete (app/api/admin/tag-settings/route.ts).
                           ⚠ schema.prisma declares @@index([userId]) and @@index([roleSlug])
                           with no `map:` — Prisma's default names (…_userId_idx /
                           …_roleSlug_idx), full not partial. Live has neither: the real
                           indexes are the partial _user_idx / _role_idx above.
                           Who sees a tag: CLAUDE_UI.md §57; MO tag-gating CLAUDE_MAIL_ORDERS.md §21.
...
### 7.11 Trip Report (v27.8 · mirror rewritten in v27.24)
...
trip_report                Standalone Supabase mirror of NTS trip/delivery data — read-only,
                           ...
                           own row id — STABLE and globally unique, NOT used for dedup).
                           UNIQUE (deliveryNo, disDate) — trip_report_delivery_no_dis_date_key.
                           Indexes (LIVE names, neither is Prisma's default, neither carries
                           a map: yet): trip_report_disdate_idx (disDate) and
                           trip_report_tripno_idx (disDate, tripNo).
mirror_heartbeat           v27.24. Liveness for external mirrors, 7 cols, one row per mirror.
                           id TEXT @id (seeded 'trip_report'), lastRunAt / lastChangedAt
                           Timestamptz(6), rowsSeen / rowsChanged / rowsRemoved / rowsSkipped
                           Int. Zero relations, no RLS.
```

Full **39**-column list: `CLAUDE_TRIP_REPORT.md §3` — 38 until `rowHash` was added in v27.24. Populated by an external PowerShell puller (outside the repo) via the `mirror_trip_report_today(rows jsonb)` Postgres function.

🔴 **CORRECTED 2026-09-08, both halves of what this block used to say.** (a) It said `sourceId` "changes per pull" — **it does not**: 74/74 identical across two consecutive live cycles, 6,779 rows carrying 6,779 distinct ascending ids with no cross-day overlap. That false claim, which also sat in `CLAUDE_TRIP_REPORT.md §2/§3`, was the stated premise of the whole 2026-07-06 design, and it is what decides whether an upsert is safe — do not rebuild on it. (b) It said the function is "an atomic per-day delete+insert, not a row-level upsert". **Since v27.24 it IS a row-level upsert**, gated on `trip_report."rowHash"` (md5 of the incoming row minus `sourceId`), preceded by a delete of only the rows the batch no longer contains, scoped to every date in the batch. Delete-and-refill was the July 2026 answer and was removed for cost — it rewrote every row every ~62s round the clock. Full mechanism, and why the function returns the deduped count rather than the changed count: `CLAUDE_TRIP_REPORT.md §2`.

⚠ **`trip_report."fetchedAt"` is no longer a liveness signal.** It now moves only when a row actually changed. "Is the puller alive" is `mirror_heartbeat."lastRunAt"`; "is the data moving" is `lastChangedAt`. Also new in v27.24: `trip_mirror_ping(p_rows integer) returns void`, which stamps liveness and does no mirror work — built for the PowerShell side and **currently uncalled**, because 🔴 **the host running the live puller is UNKNOWN** (`CLAUDE_TRIP_REPORT.md §7`).

`/trips` access: `CLAUDE_TRIP_REPORT.md §1`; roles: §5 above.
...
orders — NEW index         orders_updatedAt_idx  =  @@index([updatedAt(sort: Desc)])
                           ...
                           Other secondary indexes on orders: idx_orders_ishidden
                           (sql/2026-06-12-hide-feature.sql:35, v27.6), the partial
                           orders_billing_pending_idx (v27.13, not modelled),
                           orders_invoiceNo_idx (v27.21, @@index([invoiceNo])) and
                           orders_tripDropId_idx (v27.29, @@index([tripDropId])) —
                           besides the PK and the obdNumber UNIQUE.
```
...
### Coverage — 49 wired, 14 excluded, 9 deferred (recounted 2026-09-19)

**Counted from the tree, not from a prior claim.** `grep -rln "logAdminAction(" app` returns **49 files** on 2026-09-19 (43 on 2026-09-04). The six added since: the four Tint routes that were deferred (`74c51869`, 2026-09-06) and two routes that post-date the census — `admin/access/[userId]` (`187980cb`, the Access screen's save) and `admin/tag-settings` (`abd495f4`).
...
| Access | `187980cb` | **1** | `admin/access/[userId]` — every save of `/admin/access` |
| Tint | `74c51869` | **4** | `tint/manager/reorder`, `tint/manager/challans/[orderId]`, `tint/operator/tinter-issue/[id]`, `tint/operator/tinter-issue-b/[id]` |
| Hide | `abd495f4` | **1** | `admin/tag-settings` |
| | | **49** | |
...
**Deferred on purpose — 9 routes still open (13 on 2026-09-04; the Tint session wired four of its five).** Sampling Library 3 (`[samplingNo]` PATCH, `/review` POST, `/variants` POST) · MRN 2 (`[mrnId]/header` PATCH, `[mrnId]/lines` PUT) · Tint 1 (`operator/shades/[id]` PUT — no in-app caller) · Billing 1 (`billing/mail-order/actions` POST) · backfill 2 (`admin/fix-challans` POST, `admin/fix-slots` POST). Verified present and unwired 2026-09-19.

🔴 **These are deferred by DECISION, not oversight, and the reason is that wiring them now means editing the same files twice.** Each belongs to a module with its own conversion still ahead of it — the Sampling Library, MRN, Tint and Billing sessions each reopen these exact routes. An audit call added today would be re-read, re-reasoned and possibly re-shaped by that session anyway, so it is folded into the conversion instead. **A later session finding one of these unwired has NOT found a gap to close on its own** — it has found work that belongs to that module's session. If you are already in that module's session, wire it then.
...
**The conventions the wired routes share**, set by the three above and followed by the rest:
...
user_page_access           Per-user page permissions. camelCase, no @map. 10 columns:
                           ...
                           userId      Int      FK → users.id, ON DELETE CASCADE
                           pageKey     String   one of the 39 ALL_PAGE_KEYS values (PageKey)
...
**At the 2026-09-04 fill: 1,053 rows = 39 users × 27 page keys**, every user getting every key, active or not — a deactivated person keeps their ticks so reactivating them restores what they had. **The table is no longer dense.** The twelve keys added since were filled by their own SQL, and some wrote rows only for the people granted: live 2026-09-18 (Q03a) `billing_picking` has 5 rows, `place_order_ship_to` 5, `tint_panel_*` 3 each, `reports_*` 3 each, while `mrn`, `ci` and `floor` have 39 and the billing action ticks and `billing_print` 40. **An absent row ≡ all false** — `userPagePerms()` returns `ALL_FALSE` when `findUnique` finds nothing (`lib/permissions.ts:665-671`), and `userAllPerms()` deliberately does not densify (`:700-704`).
...
**How it was filled, and why that matters more than the table.** The fill reproduced `getAllPermissionsForRoles()` exactly — the admin short-circuit, the OR-merge across every role in `user_roles`, and `lib/auth.ts:221`'s rule that a user with ANY `user_roles` rows gets *exactly those* rows and not their primary. A parity SELECT then compared all 1,053 rows against what the live role system granted: **0 diffs**. That is why flipping the switch (§7.15 / §5) was a no-op for every person.
...
"May administer OrbitOMS." Replaces the `admin` **job title** as the thing the admin gates check (68 on 2026-09-04; 70 call sites in 60 files by grep, 2026-09-19). Live 2026-09-04: **1 of 39 users** — u1, who also still holds the `admin` role.
...
### 7.16 Trip module + app settings (v27.28 – v27.35)

**CORE owns the schema facts only.** What a trip is, its lifecycle, numbering, drops, the pick gate, Send to billing and the rule that no trip action changes a bill's status or hold → **`CLAUDE_FLOOR_TRIPS.md`** (§3 schema narrative, §4-§14 behaviour). The Print tab that reads `sentToBillingAt`/`billingCopiedAt` → `CLAUDE_BILLING.md §7`. ⚠ `trips` is **Orbit's own truck plan** — NOT `trip_report`, the read-only NTS mirror (§7.11); the collision is spelled out in `CLAUDE_FLOOR_TRIPS.md §2`. All four tables were created by hand in the Supabase SQL Editor and hand-mirrored into `prisma/schema.prisma` (§3). camelCase, no `@map`.

```
trips                      One load leaving the depot. 30 columns (24 at v27.29 + shownAt/ById
                           v27.34 + sentToBillingAt/ById, billingCopiedAt/ById v27.35).
                           id, tripNumber UNIQUE, tripDate DATE, typeCode, deliveryTypeId FK,
                           seq, dispatchWindowId FK?, transporterId FK?, vehicleId FK?,
                           adhocVehicleNo?, driverName?, driverPhone? (SNAPSHOTS),
                           transporterTripNo?, note?, status DEFAULT 'draft',
                           releasedAt/ById, dispatchedAt/ById, cancelledAt/ById, shownAt/ById,
                           sentToBillingAt/ById, billingCopiedAt/ById, createdAt, createdById,
                           updatedAt (@default(now()) @updatedAt).
                           + vehicleSize TEXT? (gc|ace|big, chk_trips_vehicle_size —
                           sql/2026-09-21-trips-vehicle-size.sql, ran live, NOT YET MINTED: §7
                           known gap) and isHand BOOLEAN NOT NULL DEFAULT false (v27.41 — a Hand
                           trip: the dealer collects, no vehicle, stays 'draft' for life).
                           SEVEN named relations to users, all named on BOTH sides:
                           TripCreatedBy / TripReleasedBy / TripDispatchedBy / TripCancelledBy /
                           TripShownBy / TripSentToBillingBy / TripBillingCopiedBy.
                           Plus TripDispatchWindow → dispatch_slot_master (named).

trip_drops                 One STOP (a customer, not a bill) on a trip. 11 columns.
                           id, tripId FK (CASCADE), dropSeq, customerId FK?, shipToCode,
                           dropKey ('c:<customerId>' | 's:<shipToCode>', written by the app),
                           customerName, areaName?, routeName?, note?, createdAt.
                           orders.tripDropId → trip_drops.id (SetNull) is the ONLY bill→trip
                           pointer (§7.3).

trip_activity              A trip's own history. 7 columns. id, tripId FK (RESTRICT — a trip
                           with history cannot be deleted), action, actorId FK → users
                           @relation("TripActivityActor"), summary, detail JSONB?, createdAt.
                           Vocabulary owner: TRIP_ACTIONS in lib/trips/activity.ts.

app_settings               Per-FLAG switches, default OFF (absent row = off). 5 columns.
                           id, settingKey UNIQUE, isEnabled DEFAULT false, updatedById → users
                           @relation("AppSettingsUpdatedBy"), updatedAt. The only key in code is
                           'picking.visibilityGate' (lib/picking/visibility-gate.ts:58 reads,
                           app/api/floor/pick-gate/route.ts:108 writes). Live 2026-09-18 (Q04):
                           isEnabled=false. ⚠ NOT app_tag_settings (per-BADGE, default ON, §7.10).
                           ⚠ The schema.prisma header still says "NOTHING CONSUMES THIS YET" —
                           a stale comment.
```

**Live constraints and indexes, verbatim (2026-09-18, Q05a/Q05b).** None of the CHECKs, and not the partial unique index, are expressible in Prisma — they exist only here and in the `schema.prisma` comments. A new status or action value needs a SQL ALTER on its CHECK **first** (§3's status-string rule).

```
trips.chk_trips_status: CHECK ((status = ANY (ARRAY['draft'::text, 'released'::text, 'dispatched'::text, 'cancelled'::text])))
trips.chk_trips_type_code: CHECK (("typeCode" = ANY (ARRAY['L'::text, 'U'::text, 'I'::text, 'C'::text])))
trips.chk_trips_seq_positive: CHECK ((seq >= 1))
trips.chk_trips_vehicle_one_of: CHECK ((NOT (("vehicleId" IS NOT NULL) AND ("adhocVehicleNo" IS NOT NULL))))
trips.chk_trips_dispatched_complete: CHECK (((status <> 'dispatched'::text) OR (("dispatchedAt" IS NOT NULL) AND ("dispatchedById" IS NOT NULL))))
trips.chk_trips_cancelled_complete: CHECK (((status <> 'cancelled'::text) OR (("cancelledAt" IS NOT NULL) AND ("cancelledById" IS NOT NULL))))
trips.chk_trips_number_shape: CHECK (
CASE
    WHEN (status = 'cancelled'::text) THEN ("tripNumber" ~ (((((('^'::text || "typeCode") || '-'::text) || to_char(("tripDate")::timestamp with time zone, 'YYMMDD'::text)) || '-'::text) || lpad((seq)::text, GREATEST(2, length((seq)::text)), '0'::text)) || '-C([2-9]|[1-9][0-9]+)?$'::text))
    ELSE ("tripNumber" = (((("typeCode" || '-'::text) || to_char(("tripDate")::timestamp with time zone, 'YYMMDD'::text)) || '-'::text) || lpad((seq)::text, GREATEST(2, length((seq)::text)), '0'::text)))
END)
trips.trips_tripNumber_key: UNIQUE ("tripNumber")
trips.trips_createdById_fkey: FOREIGN KEY ("createdById") REFERENCES users(id)
trips.trips_releasedById_fkey: FOREIGN KEY ("releasedById") REFERENCES users(id) ON DELETE SET NULL
trips.trips_dispatchedById_fkey: FOREIGN KEY ("dispatchedById") REFERENCES users(id) ON DELETE SET NULL
trips.trips_cancelledById_fkey: FOREIGN KEY ("cancelledById") REFERENCES users(id) ON DELETE SET NULL
trips.trips_shownById_fkey: FOREIGN KEY ("shownById") REFERENCES users(id) ON DELETE SET NULL
trips.trips_sentToBillingById_fkey: FOREIGN KEY ("sentToBillingById") REFERENCES users(id) ON DELETE SET NULL
trips.trips_billingCopiedById_fkey: FOREIGN KEY ("billingCopiedById") REFERENCES users(id) ON DELETE SET NULL
trips.trips_deliveryTypeId_fkey: FOREIGN KEY ("deliveryTypeId") REFERENCES delivery_type_master(id)
trips.trips_dispatchWindowId_fkey: FOREIGN KEY ("dispatchWindowId") REFERENCES dispatch_slot_master(id)
trips.trips_transporterId_fkey: FOREIGN KEY ("transporterId") REFERENCES transporter_master(id)
trips.trips_vehicleId_fkey: FOREIGN KEY ("vehicleId") REFERENCES vehicle_master(id)
CREATE UNIQUE INDEX trips_date_type_seq_live_key ON public.trips USING btree ("tripDate", "typeCode", seq) WHERE (status <> 'cancelled'::text)
CREATE INDEX trips_date_idx ON public.trips USING btree ("tripDate" DESC)
CREATE INDEX trips_status_idx ON public.trips USING btree (status)
CREATE INDEX trips_vehicle_idx ON public.trips USING btree ("vehicleId")

trip_drops.chk_trip_drops_key: CHECK (("dropKey" =
CASE
    WHEN ("customerId" IS NOT NULL) THEN ('c:'::text || ("customerId")::text)
    ELSE ('s:'::text || "shipToCode")
END))
trip_drops.chk_trip_drops_seq_positive: CHECK (("dropSeq" >= 1))
trip_drops.trip_drops_tripId_fkey: FOREIGN KEY ("tripId") REFERENCES trips(id) ON DELETE CASCADE
trip_drops.trip_drops_customerId_fkey: FOREIGN KEY ("customerId") REFERENCES delivery_point_master(id)
trip_drops.trip_drops_trip_key_key: UNIQUE ("tripId", "dropKey")
trip_drops.trip_drops_trip_seq_key: UNIQUE ("tripId", "dropSeq")
CREATE INDEX trip_drops_trip_idx ON public.trip_drops USING btree ("tripId")
CREATE INDEX trip_drops_customer_idx ON public.trip_drops USING btree ("customerId")

trip_activity.chk_trip_activity_action: CHECK ((action = ANY (ARRAY['created'::text, 'bills_added'::text, 'bills_removed'::text, 'vehicle_changed'::text, 'details_changed'::text, 'cancelled'::text, 'released'::text, 'dispatched'::text, 'renamed'::text, 'shown'::text, 'taken_back'::text, 'sent_to_billing'::text, 'taken_back_from_billing'::text, 'invoices_copied'::text])))
trip_activity.trip_activity_trip_fkey: FOREIGN KEY ("tripId") REFERENCES trips(id) ON DELETE RESTRICT
trip_activity.trip_activity_actor_fkey: FOREIGN KEY ("actorId") REFERENCES users(id)
CREATE INDEX trip_activity_trip_idx ON public.trip_activity USING btree ("tripId", "createdAt")
CREATE INDEX trip_activity_created_idx ON public.trip_activity USING btree ("createdAt" DESC)
```

**Four things that will bite:**
- 🔴 **`trips_date_type_seq_live_key` is PARTIAL** (`WHERE status <> 'cancelled'`), so a cancelled trip gives its seq back. `schema.prisma` deliberately carries **no** `@@unique` for it — modelling one would describe a full unique that does not exist (same class as `orders_billing_pending_idx`, §7).
- 🔴 **No `loading` status.** It was dropped from `chk_trips_status` in v27.31; it comes back only by ALTER, never by code assuming it.
- ⚠ **Not in the live read, taken from `schema.prisma` instead:** `orders_tripDropId_fkey` (ON DELETE SET NULL) and `orders_tripDropId_idx`; `app_settings_settingKey_key` and `app_settings_updatedById_fkey`; the `OrderPickVisibleBy` / `OrderLoadedBy` FK names on `orders`.
- ⚠ **`orders.pickVisibleAt/ById` and `orders.loadedAt/ById` are present, deliberately unused** (§7.3). Nothing reads or writes them; do not wire a feature to them without an owner decision.

### 7.17 Route clubs (v27.36)

**CORE owns the schema facts only.** What a club does on screen — the route cards, the order, the reach, the tab rules → **`CLAUDE_FLOOR.md §2.1`**. Created by `sql/2026-09-19-route-clubs.sql`, seeded by `sql/2026-09-19-route-clubs-local.sql` (one `DO` block, route and delivery-type ids looked up BY NAME, all-or-nothing). Only reader: `getRouteClubs()`, `lib/floor/route-clubs.ts`.

```
route_clubs                A set of routes that share one truck on a light day. 5 columns.
                           id SERIAL, deliveryTypeId FK → delivery_type_master (RESTRICT),
                           name (the card heading, e.g. "Adajan + Olpad"), sortOrder (the
                           card's place for its type, ≥ 1), createdAt timestamptz(6) DEFAULT now().
                           UNIQUE (id, deliveryTypeId)      — the TARGET of the members' composite
                                                              FK; looks redundant beside the PK and
                                                              is load-bearing. Never drop it.
                           UNIQUE (deliveryTypeId, name)
                           UNIQUE (deliveryTypeId, sortOrder)

route_club_members         One route in a club. 5 columns.
                           id SERIAL, clubId, deliveryTypeId (a COPY of the club's type),
                           routeId FK → route_master (RESTRICT), sortOrder (1 = main, 2 = sub, …).
                           FK (clubId, deliveryTypeId) → route_clubs (id, deliveryTypeId)
                              ON DELETE CASCADE             — a member is part of its club, and
                                                              the copied type can never disagree
                                                              with the club's.
                           UNIQUE (deliveryTypeId, routeId) — THE RULE: a route is in at most ONE
                                                              club PER DELIVERY TYPE.
                           UNIQUE (clubId, sortOrder)
```

Live 2026-09-19: three Local clubs (ids 1–3) with six members — Adajan (9) + Olpad (24), Ghod Dod (6) + Udhana (7), Varachha (8) + Kamrej (19).

**Three things to know:**
- 🔴 **A club belongs to a delivery type because a route does not have one.** A route's type lives on `area_master`, and six routes have areas in both Local and Upcountry — so the same route may sit in one Local club and a different Upcountry club. That is why the type is on the club, and COPIED onto each member: a unique can only see its own table's columns.
- ⚠ **Membership is not checked against areas, on purpose.** Kamrej has no Local area and sits in a Local club (owner, 2026-09-19); Floor draws its bills from its other tab (`reachFrom`, `CLAUDE_FLOOR.md §2.1`).
- ✅ **Adding a club, or a route to one, is a SQL insert — no code, no deploy.** Insert the club into `route_clubs` (its type and `sortOrder`), then its routes into `route_club_members` with the SAME `deliveryTypeId`; the composite FK rejects a mismatch. It shows on the next board load. Prisma: `route_clubs` / `route_club_members` hand-mirrored in `schema.prisma` with explicit `map:` names; `delivery_type_master.routeClubs` and `route_master.clubMemberships` back-relations are single FKs, so unnamed.

### 7.18 Load plan config (v27.37)

**CORE owns the schema facts only.** What the rules do — the Upcountry Load plan view, the packing algorithm, Make trip → **`CLAUDE_FLOOR.md §2.2`**. Created and seeded by `sql/2026-09-19-load-plan-config.sql` (one `DO` block, route and delivery-type ids looked up BY NAME, all-or-nothing). Only reader: `getLoadPlanPayload()`, `lib/floor/load-plan-config.ts`; validator: `parseLoadPlanConfig`, `lib/trips/load-plan.ts`.

```
load_plan_config           The Load plan's rules, ONE ROW PER DELIVERY TYPE. 3 columns.
                           deliveryTypeId integer PK + FK → delivery_type_master (RESTRICT),
                           config jsonb NOT NULL (CHECK: a JSON object),
                           updatedAt timestamptz(6) DEFAULT now() — plain @default(now()), NOT
                           @updatedAt: the table is written by SQL only.
config                     { "smallMaxKg": 2000, "bigMaxKg": 3000,
                             "mainRouteIds": [routeId, …],
                             "partners": [ { "routeId": n, "joins": [routeId, … in priority order],
                                             "pick": "most_space" | "in_order" } ] }
                           ROUTE IDS, never names. Seeded for Upcountry: main Navsari 11, Vapi 12,
                           Bharuch 17; partners Chikhli 21 → [12, 11] most_space · Vansda 13 →
                           [11, 12] · Bardoli 14 → [11] · Kamrej 19 → [17] (ids as of 2026-09-19).
```

- ✅ **Changing a rule is an UPDATE of this row — no code, no deploy.** A new delivery type gets a row of its own; the view appears for a tab only where code offers it (`LOAD_PLAN_SCOPES`, Upcountry today).
- ⚠ **A bad row is not an error.** `parseLoadPlanConfig` rejects anything malformed (a non-integer kg, small > big, an unknown `pick`) and the view reads "Load plan not set up"; the reader also survives the table not existing. Nothing in the database checks the jsonb's inside — only that it is an object.
- Prisma: `load_plan_config` hand-mirrored with an explicit `map:` on the FK; `delivery_type_master.loadPlanConfig` is the optional one-to-one back-relation (single FK, unnamed).
- **v27.38 added keys to the Upcountry row** (all earlier keys kept): `routeSides` {routeId: "South" | "North" | "Surat" | "Direct"}, `vehicles` {ace, big, gc: maxKg, overKg, idealStops, maxStops, dailyCount, nearOnly, priority}, `maxPlacesPerTruck`, `pairMinTimes`, `bulkKg`, `truckPenaltyRs`, `holdSmallUnlessOverdue`, `newArea`. The current engine (`parseLoadPlanConfig`) reads only the original four and ignores the rest — checked 2026-09-21: a config carrying the new keys parses to the identical result. They are for the v2 engine (§7.19).

### 7.19 Load plan v2 — area rates and area pairs (v27.38)

Created and filled by `sql/2026-09-21-load-plan-v2.sql`, which is **generated** from the three CSVs in `docs/data/load-plan/` (`load_plan_area_rates.csv` 269 rows, `load_plan_area_pairs.csv` 1,317 rows, `load_plan_route_sides.csv` 17 rows) — regenerate it from the CSVs rather than hand-edit it. 🔴 **The SQL file and the rates CSV are deliberately NOT in git** — both carry rates, and the GitHub repo is public. They live only on the owner's machine and are `.gitignore`d (both exact paths, plus `docs/data/load-plan/*rates*` so no future rates file can be committed by accident). The pairs and sides CSVs ARE committed — they hold no rupee figure. A CSV row naming an area not in `area_master` is SKIPPED and listed by the file's check. **Nothing reads either table yet** (grep, 2026-09-21: the SQL file is the only hit); the v2 engine step adds the Prisma models and the reader.

```
load_plan_area_rate        What a trip to an area costs, per vehicle. ONE ROW PER AREA. 11 columns.
                           areaId integer PK + FK → area_master (ON DELETE CASCADE — part of its area),
                           ratePlaceKey text NOT NULL (the rate card's place, e.g. "TITHAL"),
                           gcRate / aceRate / bigRate integer NULL (whole rupees; NULL = no rate),
                           gcExtra / aceExtra / bigExtra integer NULL,
                           gcAllowed boolean NOT NULL DEFAULT false (may a GC go there),
                           source text NOT NULL — CHECK chk_load_plan_area_rate_source:
                             'rate_card' | 'route_estimate' | 'no_rate',
                           updatedAt timestamptz(6) DEFAULT now().

load_plan_area_pair        Two areas that have shared a truck, and how often. 3 columns.
                           areaIdA, areaIdB integer — both FK → area_master (ON DELETE CASCADE),
                           PRIMARY KEY (areaIdA, areaIdB), CHECK chk_load_plan_area_pair_order
                           (areaIdA < areaIdB — one row per pair, one order),
                           timesTogether integer NOT NULL (CHECK ≥ 0).
                           Index load_plan_area_pair_b_idx (areaIdB) for the other half of
                           "pairs of area X". The engine counts a pair only at
                           timesTogether ≥ config.pairMinTimes (2).
```

- 🔴 **RATES ARE INTERNAL AND MUST NEVER REACH THE BROWSER.** No API route, server action, page, component prop or client payload may return or carry any rate or cost field — `gcRate`, `aceRate`, `bigRate`, `gcExtra`, `aceExtra`, `bigExtra`, `ratePlaceKey`, `truckPenaltyRs`, or anything computed from them (a truck's cost, a saving, a rupee figure). Rates are read and used **only on the server**; what leaves the server is the plan (which bills on which truck, the vehicle, the reason in words), never a rupee amount. ✅ **Safe today:** `GET /api/floor/board` carries `loadPlan.configs`, but `getLoadPlanPayload` (`lib/floor/load-plan-config.ts`) sends only what `parseLoadPlanConfig` returns — the four original keys — so `truckPenaltyRs` and the other new keys never leave the server (checked 2026-09-21). ⚠ **Keep it that way:** when the v2 engine parses more keys, the payload must be an explicit browser-safe subset, never the whole row.
- ✅ **A new area with no rate row** (`config.newArea`): it uses its route's **typical rate** (`"rate": "route_typical"`), **GC is not allowed** there (`"gcAllowed": false`), and it **may pair with any area on the same route** (`"pairs": "same_route"`). The v2 engine applies this; nothing does yet. On 2026-09-21 every one of the 176 live Upcountry areas has a rate row.
- ⚠ **Two FKs from `load_plan_area_pair` to `area_master`.** When the Prisma model is added, BOTH relations must be named on both sides (the §7.3 ambiguity rule) — an unnamed pair is a hard generate error.
...
**`applyMailOrderEnrichment()`:** On SAP import, checks `mo_orders` for matching `soNumber`. If found, applies `dispatchStatus`, `priorityLevel`, `remarks`, overrides, and sets `orderDateTime` from `mo_orders.receivedAt`. Skips slot recalc for tint orders. One soNumber can map to many OBDs (`updateMany`). Bills that no mail order matched are then released by `applyNoMailOrderFallback` (non-tint, `dispatchStatus` NULL → `pending_picking` + engine slot) — owner `CLAUDE_IMPORT.md §2.1`.
...
### Admin → Access
**`/admin/access`** (2026-09-04). Superuser only. **The screen that decides what everybody can do** — per-user page permissions, one person at a time: a searchable people rail on the left, and on the right that person's 39 page keys (the `PageKey` union, in the 5 `ACCESS_SECTIONS` groups) × 5 actions as a `CLAUDE_UI.md §27` fixed table, with a dash wherever `isActionAvailable()` says the app never asks that action (§5). Writes `user_page_access` (§7.14); every save writes one `admin_audit_log` row (§7.13). Shows which source is live (`ACCESS_SOURCE`, §5) in a banner, and an amber dot wherever a person's ticks differ from what their job title would grant. → `CLAUDE_UI.md §63`.

### Mail Orders — sidebar label "Billing"
`/mail-orders`, page key `mail_orders` (who holds it: per-user ticks, §5). The Billing desk — rollout flag, tab bar, Picking tab, Print tab, action ticks, marker providers, notes font size → **`CLAUDE_BILLING.md`**; the parser, enrichment and Orders-tab internals → `CLAUDE_MAIL_ORDERS.md`.

### Tint Manager / Operator / Challans / Shades
`/tint/*`. → `CLAUDE_TINT.md`

### Reports
`/reports` hub + `/reports/tint-summary`. Gated per report by `REPORT_PAGE_KEYS` (`reports_tint_summary`, `reports_ti_report`): canView on any one opens the hub (`app/reports/page.tsx:42`), and each report shows only to its own key's holders (§5). The old `ti_report` key gates nothing since `6f628b05` (2026-09-17). **Tint Summary** daily MIS report + the former **TI Report** (folded in — old `/tint/manager/ti-report` and `/ti-report` redirect to `/reports?r=ti-report`). API: `GET /api/reports/tint-summary`. → `CLAUDE_TINT.md §11-§12`, `CLAUDE_UI.md §56`.
...
`/po2` (public v2 mobile order page), `/po9` (the same app with ship-to off) and `/po-v2-8f4kd2` (a `redirect("/po2")` stub for old bookmarks) → **`CLAUDE_PO2.md`**.

### Orders
`/orders` — a one-line `redirect("/floor")` (`app/orders/page.tsx`). Public only because of the `/order` prefix (below); the redirect then meets `/floor`'s own gate.
...
### Floor Control
`/floor`, page key `floor` (per-user ticks, §5). **The trip desk** (`components/floor/trip-desk.tsx`, since `bbb9628c`, 2026-09-10): trips on the left rail, bills on the right, with Floor / Tinting / On hold / Cancelled tabs and the detail panel — screen → `CLAUDE_FLOOR.md §2`; trips, the pick gate and Send to billing → `CLAUDE_FLOOR_TRIPS.md`; schema §7.16. The earlier decision rail (undecided bills) stopped rendering 2026-09-10 and was archived 2026-09-13 (`79bcc412`, `archive/2026-09-floor-rail/`, `CLAUDE_FLOOR.md §9c`). Hand-rolled header (UI §6 named exception). Built to consolidate the Support board + the Picking **desktop** board; **both are now retired** — Support 2026-07-27 (`archive/2026-07-support/`, `CLAUDE_FLOOR.md §9`) and the Picking desktop queue 2026-07-28 (`archive/2026-07-picking-desktop/`, `CLAUDE_FLOOR.md §9b`). **`/picking` itself is still live** and keeps its card boards at every width. → `CLAUDE_FLOOR.md`.
...
**`/ci` is in the sidebar and the phone Menu.** `{ pageKey: "ci", label: "CI", href: "/ci" }` sits in `PAGE_NAV_MAP` at `lib/permissions.ts:102` (added by `55c3cdc6`, 2026-08-31), directly after `mrn`, with the icon `Undo2` (`components/shared/role-sidebar.tsx:76`). Its position is behaviour: `MobileShell`'s phone Home is `navItems[0]?.href`, and the comment above the line (`:86-101`) records that `navItems[0]` was re-derived on 2026-09-01 and is unchanged for `floor_supervisor` (`/picking`), `billing_operator` (`/place-order`) and `operations`. Re-derive it before moving the line. Was described here as "reachable by URL only" from 2026-09-03 to 2026-09-19; that was never true of the code. Do not revert.
...
- `/po2`, `/po9`, `/po-v2-8f4kd2` — public through the `/po` prefix (below). → `CLAUDE_PO2.md`.
- `/demo` — animated tutorial. Rewrites to `/order-demo.html`.
- `/login`, `/not-ready`, `/unauthorized`.

`middleware.ts` public paths: `/login`, `/unauthorized`, `/not-ready`, `/api/auth`, `/api/health`, `/order`, `/api/order`, `/po`, `/demo`, `/order-demo.html`, `/api/cron/*` (bearer auth). ⚠ **`/order` stays on purpose** even though its page is retired: the check at `middleware.ts:36` is `startsWith`, so removing it would (a) make the parked address return a **login prompt** instead of a 404, and (b) silently put **`/orders`** behind auth via the prefix match. *(The code comment at `middleware.ts:20` still says "line 26" — stale.)*

🔴 **The prefix match makes more public than the list says.** `"/po"` also passes **`/po2`, `/po9` and `/po-v2-8f4kd2`** (deliberately — `app/po9/page.tsx:12-13` relies on it; `CLAUDE_PO2.md §4`). `"/api/order"` also passes **`/api/orders/*`**, so the three `/api/orders/[id]/*` routes skip the session redirect and are protected only by their own checks: `audit-history` (`auth()` → 401, then `tint_panel_details`), `detail` and `removed-lines` (`auth()` then `requireRole`, whose `redirect()` covers a null session). Never give a private route a name starting `/po`, `/order`, `/api/order` or `/demo`.
...
> `docs/prompts/archive/2026-07/code-discovery-2026-07-19b-catalog-repoint.md`.
...
- **~~22 sites still compare `session.user.role !== "admin"` as an admin BYPASS of a permission check~~ — REMOVED 2026-09-06, and the warning attached to them was WRONG ON ITS OWN TERMS.** The 57 remaining one-clause wrappers (`if (!roles.includes("admin")) { checkAnyPermission(...) }` and its singular spelling) are gone; the check they wrapped now runs unconditionally. **The removal is provably behaviour-neutral, and not merely "neutral today":** `checkAnyPermission`'s first line is `if (roleSlugs.includes("admin")) return true` and `checkPermission`'s is `if (roleSlug === "admin") return true` — testing *the same value by the same test* the wrapper tested — and both then check `isSuperuser` immediately after. A wrapper could only ever skip a call that would have returned `true`. The commit proves it structurally: 266 lines removed, 152 added, and the 114-line difference is exactly 57 wrapper-opens plus 57 closing braces, with **not one line introduced** — every added line is a dedented copy of a removed one.
  - 🔴 **The old warning — "a superuser who is not ALSO role-admin would fall through into the permission check and would be denied" — was already false when it was written.** Such a person fell into the resolver and was admitted by the `isSuperuser` arm two lines later. The 2026-09-04 resolver change (§5) closed that gap at the same moment the entry describing it was added; the entry described a risk that no longer existed. Corrected here rather than deleted, because "the second superuser is unsafe" is exactly the kind of claim that gets quoted forward.
  - **The count was also wrong, twice over.** "22" counted mutating routes only and excluded tint. Swept fresh across `app/` on 2026-09-06: **76 admin-test occurrences**, of which **57** were one-clause bypass wrappers (all removed), **9** were MULTI-clause conditions that also bypass for `tint_manager`/`support`/`operations` — those are permission **grants written as role names**, not bypasses, and every one of them changes somebody — **4** were the same redundancy in a variable form (`const isAdmin = …`), **1** was `(ops)/layout.tsx:37`'s un-negated FACE branch (two different chrome trees — **must not be touched**), and the rest were comments or UI flags.
  - **Still open — ONE.** (a) `app/api/mrn/photo/[photoId]/route.ts:167-168` — `const isAdmin = roles.includes("admin"); const canDelete = isAdmin || (await checkAnyPermission(roles, "mrn", "canDelete"))`. Same redundancy, expression form rather than wrapper, so the sweep did not match it. Harmless for the same reason as the 57. *(This entry read `:169-170` until 2026-09-06; the lines are 167-168 — read them, do not trust the anchor.)* (b) **CLOSED 2026-09-17 (`6f628b05`).** `app/api/reports/tint-summary/route.ts` carried the last multi-clause grant written as a role name (`role !== "admin" && role !== OPERATIONS` behind a `requireRole` list); it now gates on `checkAnyPermission(roles, "reports_tint_summary", "canView")` (`:37`) before the `try`, returning a JSON 401/403. `grep` for `role !== "admin"` across `app lib components` returns 0 hits. Operations User reaches the report because he holds that tick (live 2026-09-18 Q03b), not because of his role.

- 🔴 **THE THREE ADMIN GETs THAT DELIBERATELY GATE ON THE "WRONG" KEY — do not "fix" them** [2026-09-06, `d3211766`]. `admin/areas` **GET**, `admin/routes` **GET** and `admin/sub-areas` **GET** are gated on **`customers`/`canEdit`**, NOT on `routes_areas`/`canView`. This looks like a copy-paste error and is the opposite of one.
  - **The caller check is the reason.** All three have exactly **one** caller in the entire app: `components/shared/customer-missing-sheet.tsx:162-165`, the missing-customer form on the Tint Manager board. The admin Routes / Areas / Sub-areas tables do **not** call them — their list data arrives as server props and they only ever POST/PATCH. So the three are reference data for **creating a customer** (the Area dropdown `validate()` requires, its Sub-area child, the Route behind the area), not for managing routes.
  - **What the obvious key would have cost, verified live:** `routes_areas`/`canView` is held by **Harsh alone**. Chandresh Kolgha and Prakash can both open the sheet (`tint_manager`/`canView`) and both hold `customers`/`canEdit` — so the chosen key covers exactly the people who can reach the form, and the semantically obvious one would have **broken the Assign flow for both**.
  - **And granting the obvious key would have been worse than the bug.** `buildNavItems` reads the *same* tick, so a `routes_areas` grant handed out to make a dropdown work would have put a **Routes admin screen in a tint manager's sidebar** — a screen he has never had. A tick is not scoped to the caller that needed it. Each of the three routes carries this reasoning inline.

- **`sub-areas` CREATE is now looser than `sub-areas` EDIT, and that is a parked owner decision, not drift** [2026-09-06]. `admin/sub-areas` POST moved from `requireSuperuser` to `routes_areas`/`canEdit` to match its `areas` and `routes` siblings; its own `[id]` PATCH and `import` are still `requireSuperuser`. **Same person either way today** — Harsh is the sole superuser and holds every `routes_areas` flag — but a different rule tomorrow. The owner's decision was to **leave it and bring the other two forward**, not to revert the one. The block is commented so reverting that single handler is a two-line change if the decision flips.

- 🔴 **`requireRole` HAS NO ADMIN ARM. An admin-only account is excluded from every gate whose array does not spell `admin`.** `lib/rbac.ts` — `requireRole` is a plain "does the merged role set intersect `allowed[]`", with no superuser short-circuit of any kind, unlike `checkPermission`/`checkAnyPermission`, which carry both arms on their first two lines. Live consequence found 2026-09-06: `requireRole([TINT_OPERATOR, OPERATIONS])` was **redirecting the owner to `/unauthorized`** on the four tint operator writes, and he only stopped noticing because he also holds the `admin` role, which those arrays did not name either. The conversion incidentally fixed those four (`CLAUDE_TINT.md §13.2`). **Recounted 2026-09-19:** `grep -rn "requireRole(" app lib components`, excluding the definition in `lib/rbac.ts` and comments, finds **20 call sites** (58 on 2026-09-06). **All 20 name `admin`**; 19 also name another role, and `app/api/mail-orders/backfill-enrich/route.ts:169` is admin-only. So the role arm admits the owner everywhere today; the hole left is a superuser who does NOT hold the `admin` role, whom `requireRole` still refuses because it never reads the flag. Do not add an admin or superuser arm to `requireRole` as a drive-by — it would silently widen 20 gates at once; convert the call site instead.

- 🔴 **THE SILENT-403 PATTERN — one systemic bug, not three. Fix it at the helper.** `fetchAll` in `components/shared/customer-missing-sheet.tsx:118-130` returns **`[]`** on `!res.ok` and `[]` again from a bare `catch`, so **"you are forbidden" and "there is genuinely nothing here" are the same value.** Its cousins do the same thing with a `console.error`. It is made worse by `requireRole`, which fails by calling `redirect()` — a **307 that `fetch` follows into `/unauthorized`**, so the response arrives **200 with HTML**, `res.ok` is true, and the failure only surfaces when `res.json()` throws inside somebody's catch. **This produced THREE silent failures in one week**, each diagnosed separately before the shape was recognised:
  - **the manual-entry modal** — Prakash held the write and was refused the companion read; an empty box and a `console.error` nobody reads (`2b25a48f`, `CLAUDE_TINT.md §13.2`);
  - **the missing-customer sheet's dropdowns** — the three GETs above, which is why their key choice had to be checked against the caller rather than the name;
  - **the old `/admin/permissions` grid** — which recreated 12 retired page keys on save, caught only because `admin_audit_log` had just been wired (§7.13).
  **Record it as ONE issue and fix it where the swallowing happens** — a helper that cannot distinguish 403 from empty, in front of a guard that returns 200-with-HTML on denial. Patching the three call sites leaves the fourth to be found the same way. ROADMAP → User-based access.

- 🔴 **`prisma/seed.ts` HAS NO NOTION OF `user_page_access` OR `isSuperuser`** [verified by grep, 2026-09-06: zero hits for either]. It still seeds `role_permissions` — **the fallback** — and nothing else. **A wipe-and-reseed would rebuild the table nobody reads and leave live access EMPTY**, which under `ACCESS_SOURCE = 'user'` means every non-superuser resolves to all-false: the app comes back up with nobody able to do anything, and the seed will look like it succeeded. This is the same seed-is-not-live trap as the `dispatcher`/`support` drift in §5 and the picking grants above, one layer more dangerous because the *authoritative* table is the one seed has never heard of. Either teach seed the new table or make it fail loudly; do neither and the next reseed is an outage.
...
- **~~TM reorder API uses `prisma.$transaction`~~ — CLOSED 2026-09-05 (`a0f9378b`).** `app/api/tint/manager/reorder/route.ts` now runs sequential awaits; the only `transaction` left in the file is the comment at `:102` saying so.
- **Challan PATCH `prisma.$transaction`** (`app/api/tint/manager/challans/[orderId]/route.ts:551`) — formula upsert wrapped in `$transaction`. Pre-existing; only Chandresh saves challans (low concurrency). Refactor in a dedicated session.
- **Every live `prisma.$transaction` call, 2026-09-19** (`grep -rn '\$transaction(' app lib components` — 12 calls in 11 files, all violating §3, none in `lib/` or `components/`): `admin/areas/[id]:43` · `admin/customers:137` and `:194` · `admin/permissions:83` (§7.13) · `admin/shades:40` · `admin/skus:64` · `tint/manager/cancel-assignment:28` · `tint/manager/challans/[orderId]:551` · `tint/manager/splits/cancel:43` · `splits/create:150` · `splits/reassign:50` · `tint/operator/split/done:56`, all under `app/api/`. The `admin/customers:137`, `admin/shades:40` and `admin/skus:64` ones are the ARRAY form (a paired `findMany` + `count`), not an interactive transaction. Pre-existing; convert each inside its own module's session.
...
- **Pre-existing `prisma.$transaction` in admin customer routes** (`app/api/admin/customers/route.ts` lines 137 & 194) — flagged in multi-SO commit, left untouched. Refactor when convenient.
...
- **`mo_orders.updatedAt` is the OPPOSITE case — the DATABASE owns it, so code must NOT write it** [v27.26, `71e7b53a`]. Live trigger `trg_mo_orders_updated_at` `BEFORE UPDATE → EXECUTE FUNCTION set_updated_at_mo_orders()` (live 2026-09-18 Q11) stamps every update, raw SQL included. Prisma declares `@default(now())` and deliberately **no** `@updatedAt`. Adding `@updatedAt`, or an explicit `updatedAt: new Date()` copied from the `push_subscriptions` rule above, makes two writers for one column. Read which rule a table follows before stamping it: `push_subscriptions` = code stamps; `mo_orders` = trigger stamps; `user_page_access` / `app_settings` / `trips` = Prisma `@updatedAt`.
- **`/admin/access` cannot grant CI Export** [found 2026-09-19]. `app/api/ci/export/route.ts:45` gates the register export on `ci` **canExport**, but `ACTION_PAGES.canExport` (`lib/permissions.ts:453`) lists only `mrn` and `reports_ti_report`, so the access screen draws a dash on CI's Export cell. The 5 live holders (Q03a) cannot have been ticked on that screen. The fix is one entry in `ACTION_PAGES` — the map's own header asks for exactly that when a check exists; do not remove the route's check instead.
...
| `lib/billing/flag.ts` + `lib/billing/picking-where.ts` | Billing v2 rollout gate (per-page-load read of `billing_settings` + `users.billingV2TestUser`; live `ALL_USERS` since 2026-08-06, Q01) · shared WHERE builders for the billing Picking tab (list + marker read the same predicate) | `CLAUDE_BILLING.md §2/§6` |
| `lib/billing/print.ts` | The Print tab (slice 9): which trips are copyable, distinct invoice numbers, never a partial set, new-since-copy; writes only to `trips` + one activity row | `CLAUDE_BILLING.md §7` |
| `lib/dispatch/punch-clock.ts` | arrival-clock validation feeding the dispatch engine (2026-08-02/03 commits `03b6dd19`→`dee603dc`: a date-only punch is a DAY, not a clock — both arrival clocks validated before slotting) | `CLAUDE_IMPORT.md §12.1b` |
| `lib/dispatch/completion-slot.ts` | `resolveCompletionSlot()` — the one owner of "what slot does a bill get when it stops being blocked", shared by tint done, split done and the Base bypass. Touches Prisma, so it is kept out of the pure `dispatch-engine.ts`; returns data, never writes | `CLAUDE_TINT.md §2` |
| `lib/trips/*` | The trip module — 9 files: `activity.ts` (`TRIP_ACTIONS`, the `chk_trip_activity_action` twin), `billing.ts`, `drop-key.ts`, `live-trips.ts`, `number.ts`, `queries.ts`, `route-label.ts`, `show.ts`, `type-choice.ts` | `CLAUDE_FLOOR_TRIPS.md §16`; schema §7.16 |
| `lib/picking/visibility-gate.ts` | The pick visibility gate ("desk control"): `PICK_VISIBILITY_GATE_KEY`, `isPickGateOn()` (reads `app_settings`, fails closed to OFF), the waiting-branch predicate on `trips.shownAt` | `CLAUDE_FLOOR_TRIPS.md §11` |
| `lib/access/source.ts` | `getAccessSource()` — the `ACCESS_SOURCE` switch, 30 s module cache, every failure resolves to `role` | §5 |
| `lib/access/role-baseline.ts` | What a person's job title WOULD grant, for `/admin/access`'s "differs" marker — calls the switch-independent `getRolePermissionsForRoles()`, never the five resolvers | §5, `CLAUDE_UI.md §63` |
| `lib/import-qty-guard.ts` | Header-vs-lines quantity guard on the two header-sourced import paths; measures and records, does not block | `CLAUDE_IMPORT.md §8.3` |
| `lib/sap-paste/*` | The SAP clipboard-paste import: `read-paste.ts` (pure; any bad line blocks the whole paste), `resolve-names.ts` (repairs cut-off customer names by code, create path only), `index.ts` | `CLAUDE_IMPORT.md §3.4` |
...
## Change log — v111 (2026-09-24, billing CI/Hand marks — Schema v27.41)

- §7 chain: **v27.41** — `mo_orders.billOnlyAt/ById` + `handAt/ById`, `orders.handAt/ById`, `so_tags.fromMailOrder`, `trips.isHand`, partial index `mo_orders_billOnly_soNumber_idx`. SQL `sql/2026-09-24-billing-mo-actions.sql`, run live 2026-09-24 and verified (8 columns, 3 FKs SET NULL, index present, 0 rows marked). New relations `MoOrderBillOnlyBy`, `MoOrderHandBy`, `OrderHandBy`, named on both sides.
- §7 chain: **known-gap line** — `sql/2026-09-21-trips-vehicle-size.sql` and `sql/2026-09-21-load-plan-snapshot.sql` ran live but were never minted.
- §7.3 `orders`: HAND columns; users-relation count SEVEN → EIGHT. §7.6 `mo_orders`: BILLING MARKS + the partial index. §7.16 `trips`: `vehicleSize` (unminted) and `isHand`. `so_tags` has no §7 block of its own — its new column is in the v27.41 chain entry.
- Header and footer stamp → v111 · Schema v27.41.

## Change log — v110 (2026-09-22, CI source 'floor' — Schema v27.40)

- §7 chain: **v27.40** — `chk_ci_returns_source` gains `'floor'` (the Floor desk's Raise CI). Record file `sql/2026-09-22-ci-source-floor.sql`; the DDL was run live by Smart Flow first.
- Header and footer stamp → v110 · Schema v27.40. Nothing else in canon touched.

## Change log — v109 (2026-09-22, SO tags + auto_bill_only CI source — Schema v27.39)

Evidence: `sql/2026-09-22-billing-telephonic.sql`, run live 2026-09-22 by Smart Flow and verified by its own PART 5; re-read the same day, read-only, from `information_schema.columns`, `pg_constraint` and `pg_indexes` before the Prisma models were written.

- §7 chain: **v27.39** — `so_tags`, `so_tag_matches`, `chk_ci_returns_source` widened to `auto_bill_only`, the `billing_telephonic` all-false access rows.
- The feature that will use them (the Billing · Telephonic tab) is NOT built and is deliberately not documented here yet.
- Header and footer stamp → v109 · Schema v27.39.

## Change log — v108 (2026-09-21, load plan v2 step 2 — Schema v27.38)

Evidence: `sql/2026-09-21-load-plan-v2.sql` (NOT yet run live; local only, not in git — it carries rates) generated from `docs/data/load-plan/*.csv`; a read-only check 2026-09-21 that every CSV area and route id exists live; a grep showing nothing reads the new tables; a check that `parseLoadPlanConfig` returns the identical result with the new keys present.

- §7 chain: **v27.38** — `load_plan_area_rate`, `load_plan_area_pair`, and the eight new Upcountry config keys; recorded on the day, marked not-yet-run.
- §7.18: the new keys listed, and that the current engine ignores them.
- New **§7.19**: both tables; 🔴 the rule that rates never reach the browser (safe today: the board payload carries only the parsed original keys; it must stay an explicit browser-safe subset); the new-area defaults (route-typical rate, no GC, same-route pairs); the two-FK naming trap for the future Prisma model.
- Header and footer stamp → v108 · Schema v27.38.

## Change log — v107 (2026-09-19, load plan config — Schema v27.37)

Evidence: `sql/2026-09-19-load-plan-config.sql` (`ce10bfb1`, NOT yet run live), `prisma/schema.prisma` at `a034709e`, `lib/floor/load-plan-config.ts`, `lib/trips/load-plan.ts`.

- §7 chain: **v27.37** — `load_plan_config`, recorded on the day and marked not-yet-run.
- New **§7.18 Load plan config**: columns, the jsonb shape, the Upcountry seed, that a rule change is an UPDATE only, and that a bad row reads "not set up". Behaviour → `CLAUDE_FLOOR.md §2.2`.
- Header and footer stamp → v107 · Schema v27.37. Domain files not bumped (`CLAUDE.md §4` item 4).

## Change log — v106 (2026-09-19, route clubs — Schema v27.36)

Evidence: `sql/2026-09-19-route-clubs.sql` and `sql/2026-09-19-route-clubs-local.sql` as run live 2026-09-19 (their checks returned 20 rows and 6 rows as expected), `prisma/schema.prisma` at `f8647a6b`, `lib/floor/route-clubs.ts`.

- §7 chain: **v27.36** minted on the day — `route_clubs` + `route_club_members`.
- New **§7.17 Route clubs**: columns, uniques, the composite FK with ON DELETE CASCADE, the one-club-per-route-per-type rule, and that a new club is a SQL insert only. Behaviour pointer → `CLAUDE_FLOOR.md §2.1`.
- Header and footer stamp → v106 · Schema v27.36. Domain files are NOT bumped with it (`CLAUDE.md §4` item 4).

## Change log — v105 (2026-09-19, canon sweep batch C1 — CORE reconciled to code + live 2026-09-18)

Evidence: code at `856d0e71`, `git show <hash> -- prisma/schema.prisma` for every minted version, the committed SQL files, and the read-only live results of 2026-09-18 (`docs/prompts/drafts/sql-2026-09-18-canon-sweep-live-results.csv`, cited by Q-number). Plan of record: `docs/prompts/drafts/code-discovery-2026-09-18-canon-sweep.md` §3/§4.

- **§7 — eleven versions minted, v27.25 … v27.35** (notesFontSize, `mo_orders.updatedAt`, `route_master.bayNumber`, `app_settings` + `pickVisible*`, `trips`/`trip_drops` + `tripDropId`/`loaded*`/`isRealTransporter`, the `app_tag_settings` scope rework, `chk_trips_status` minus `loading`, `trip_activity`, trip-number reuse, `shownAt`, the Send-to-billing columns). v27.25–27 are August commits numbered retroactively. Header and footer stamp → **v27.35**. 🔴 Third module to ship outside the chain.
- **§7.16 new** — schema block for `trips`, `trip_drops`, `trip_activity`, `app_settings`, every live constraint and index verbatim; behaviour points to `CLAUDE_FLOOR_TRIPS.md`.
- **§7.1 / §7.3 / §7.6 / §7.10 / §7.12 / §7.13 / §7.14 / §7.15** — `role_master` 13 rows (id 17 `floor_access`); seven named user relations on `orders` + `tripDropId`, `pickVisible*`/`loaded*` marked present-and-unused; `mo_orders.updatedAt` trigger; `app_tag_settings` scope with its partial indexes (and a Prisma `@@index` name mismatch); `orders` secondary indexes listed; audit coverage 43 → 49 wired, 13 → 9 deferred; 39 keys and absent-row ≡ false; superuser gate recount.
- **§5** — 39 `PageKey`s, the seven missing keys with their gates, `REPORT_PAGE_KEYS`/`canViewAnyReport` (`ti_report` gates nothing), `ACTION_PAGES`/`isActionAvailable`, `PAGE_LABEL_OVERRIDES`, `ACCESS_SECTIONS`; the billing action ticks ARE read (`a991a1c4`); `mail_orders` labelled "Billing"; `ci` in `PAGE_NAV_MAP`; `floor_access` row; anchors.
- **§1 / §7.4 / §9** — the import auto-release (`applyNoMailOrderFallback`) replaces "Floor Control release" in the pipeline and the "103 Deco Retail parked" bullet; pointers to `CLAUDE_IMPORT.md §2.1`; engine anchors and its two further call sites.
- **§4** — the `/.well-known/assetlinks.json` rewrite; crons re-read.
- **§12** — Floor is the trip desk; CI's "URL only" block replaced (never true of the code); Reports gate on `REPORT_PAGE_KEYS`; Mail Orders → `CLAUDE_BILLING.md`; `/po2`/`/po9`/`/po-v2-8f4kd2` → `CLAUDE_PO2.md`; `/orders`; the prefix side effects of `/po` and `/api/order`; `middleware.ts:36`.
- **§13** — closed: TM reorder `$transaction`, the tint-summary role grant. Corrected: `requireRole` recount (20 sites, all name admin). Added: the full `$transaction` list, the `mo_orders.updatedAt` trigger case, CI Export ungrantable on `/admin/access`.
- **§15** — rows for `lib/trips/*`, `visibility-gate.ts`, `billing/print.ts`, `access/source.ts`, `access/role-baseline.ts`, `dispatch/completion-slot.ts`, `import-qty-guard.ts`, `sap-paste/*`; doc homes filled for billing flag and punch-clock.
...
*CORE v111 · Schema v27.41 · OrbitOMS · updated 2026-09-24 — **Billing CI/Hand marks:** v27.41 (`mo_orders.billOnlyAt/ById` + `handAt/ById`, `orders.handAt/ById`, `so_tags.fromMailOrder`, `trips.isHand`, partial index `mo_orders_billOnly_soNumber_idx`; run live and verified 2026-09-24, `sql/2026-09-24-billing-mo-actions.sql`), plus the known-gap line for the two unminted 2026-09-21 SQL files. Prior, v110 (2026-09-22) — **CI source 'floor':** v27.40 (`chk_ci_returns_source` + `'floor'`, run live 2026-09-22, recorded in `sql/2026-09-22-ci-source-floor.sql`). Prior, v109 (2026-09-22) — **SO tags:** v27.39 (`so_tags`, `so_tag_matches`, `chk_ci_returns_source` + `auto_bill_only`, `billing_telephonic` all-false access rows) run live and verified 2026-09-22; the tab that uses them is not built. Prior, v108 (2026-09-21) — **load plan v2 (step 2):** v27.38 (`load_plan_area_rate`, `load_plan_area_pair`, eight new Upcountry config keys) recorded on the day, not yet run live; §7.19 added with the rates-never-reach-the-browser rule and the new-area defaults. Prior, v107 (2026-09-19): **load plan config:** v27.37 (`load_plan_config`) recorded on the day, not yet run live, and §7.18 added. Prior, v106 (2026-09-19): **route clubs:** v27.36 minted on the day (`route_clubs` + `route_club_members`) and §7.17 added. Prior, v105 (2026-09-19): **canon sweep batch C1: eleven schema versions minted (v27.25 … v27.35) and §7.16 added for the trip module and `app_settings`; §5 brought to the 39 `PageKey`s; §12/§13/§15 reconciled to code and the 2026-09-18 live read.** Change log entry above. Prior, v104 (2026-09-08, second pass, same day) — **§4 gains the production database runtime settings, which had never been written down anywhere**, and §7.11 is corrected on both halves of what it said. **Slow queries are now logged** (`log_min_duration_statement = '2000'` on `postgres` AND `authenticator`) and **the app's role finally has a `statement_timeout`** (`'30s'` on `postgres`) — it had NONE while anon/authenticated/authenticator carried 3s/8s/8s, and a **300-second query appears in the 2026-09-07 outage log**. Both set at ROLE level because `ALTER DATABASE … SET` is REFUSED on Supabase; the escape hatch for maintenance is `SET statement_timeout = 0;` first, in the same tab. 🔴 **New landmine with its own line: restarting Supabase WIPES `pg_stat_statements`** — on 2026-09-07 the restart destroyed the only record of which query was slow, and the outage is unexplained as a direct result. Capture the diary BEFORE the restart. §4's depot-tools table also gains `Pull-TripReport.ps1`, which had never been listed in it. **§7.11 corrected twice over:** `sourceId` does **NOT** "change per pull" (74/74 identical across two live cycles; 6,779 rows, 6,779 distinct ascending ids) — that was the premise the entire 2026-07-06 delete-and-refill design rested on, and it also sat in `CLAUDE_TRIP_REPORT.md §2/§3`, fixed in the same pass; and the function is no longer "an atomic per-day delete+insert, not a row-level upsert" — since v27.24 it **is** a row-level upsert gated on `rowHash`. Column count 38 → **39**, live index names recorded (`trip_report_disdate_idx`, `trip_report_tripno_idx` — neither is Prisma's default and neither carries a `map:` yet), and the rule that matters most restated where a reader will hit it: **`fetchedAt` now means the data changed; `mirror_heartbeat."lastRunAt"` means the puller ran.** `docs/CLAUDE_TRIP_REPORT.md` goes to **v1.2 · Schema v27.24** in the same commit with all ten §8 corrections applied; `docs/ROADMAP.md`'s 38-column line corrected. **Schema stamp UNCHANGED at v27.24** — this pass is documentation only, no SQL and no schema edit. Prior, v103 (2026-09-08): **v27.24 MINTED, and minted ON THE DAY.** The trip mirror stopped rewriting unchanged rows: `trip_report."rowHash"` (md5 of the incoming row minus `sourceId`), a new `mirror_heartbeat` table, a new `trip_mirror_ping()`, and `mirror_trip_report_today` REPLACED in place — the temporary table gone, delete-and-refill replaced by a hash-gated upsert plus a delete of only what the batch no longer contains. SQL applied in the Supabase SQL Editor and verified 2026-09-08; this commit is the repo catching up to the database. The old body rewrote ~100 rows every ~62s round the clock (13,555 inserts / 13,438 deletes / **0 updates**) and created and dropped a 37-column temporary table ~1,390 times a day; steady state now writes nothing. 🔴 **One canonical claim was measured FALSE in the same pass and it was the premise the design turned on:** `CLAUDE_TRIP_REPORT.md §2/§3` say NTS reissues `sourceId` on every pull — it does not (74/74 identical across two live cycles; 6,779 rows, 6,779 distinct ids, ascending by date, no cross-day overlap). Had it been true, a frozen unchanged row could have collided with a new row on `trip_report_pkey` and the whole upsert approach would have been unsafe. Two unguarded edges in the old function were closed with it: the IST-midnight `max("disDate")` collision that would fail an entire cycle, and the NULL-`disDate` row that was insertable but never deletable. The return value stays the **deduped** count, not the changed count, so the untouched puller keeps logging `mirrored N rows for today` rather than a `0` indistinguishable from failure. ⚠ **The PowerShell side is Phase B and is BLOCKED on an unidentified host** — the live puller is not on this machine, which has no such process, no scheduled task, and a `log.txt` seven days stale while production took a write every 62 seconds. ⚠ **`CLAUDE_TRIP_REPORT.md` is NOT edited in this pass**: it owes ten corrections (puller path, the undocumented `modi_inv='FRT'` filter, the real 61–65s cadence, the two live index names, the `sourceId` falsehood, and more), listed in §8 of `docs/prompts/archive/2026-09/code-resume-2026-09-08-trip-mirror-rewrite.md`. Its schema stamp is still v27.13 and that is NORMAL, not a defect (router §4). Prior, v102 (2026-09-06): **the 2026-09-06 Tint and master-data conversions recorded** (`cd0ed055` → `fbbe30bd`, seven commits, all pushed). **Schema stamp UNCHANGED at v27.23: nothing schema-side moved.** Those seven commits ran no SQL, altered no table and touched no column — they moved gates from job titles to the ticks `user_page_access` has held since 2026-09-04, so §7 needed no edit and the stamp stays where it is. **§5 gains `ROLE_HREF_OVERRIDES` as a documented mechanism**: the same tick resolves to three different URLs by primary job title, and **11 of the 12 master-data pages render the SAME COMPONENT** — `/tint/manager/customers` uses `CustomersSplitView` exactly as `/admin/customers` does, so the three route groups are duplication, not variation. The stale "richer split view" sentence is corrected **and relocated** — it is a code comment at `lib/permissions.ts:117` and a ROADMAP bullet, both describing the *retired Support* copies; it was never in this §5, and the 2026-09-06 gate report that said it was is wrong on the location. §5 also records, SELECT-verified, that **the four `/dispatcher/*` master-data screens are reachable through the UI by NOBODY** — including by all three holders of the `dispatcher` role, who hold none of the four ticks — which is not a contradiction of §12's "they are LIVE": live and reachable are different claims. **§13 gains six entries and corrects one anchor.** 🔴 The three admin GETs (`areas`, `routes`, `sub-areas`) gate on **`customers`/`canEdit`, not `routes_areas`/`canView`, deliberately** — their only caller in the app is the missing-customer sheet on the Tint Manager board, the obvious key is held by the superuser alone and would have broken Chandresh's and Prakash's Assign flow, and *granting* it would have put a Routes admin screen in a tint manager's sidebar because `buildNavItems` reads the same tick. 🔴 **`requireRole` has no admin arm**, so an admin-only account is excluded from any gate whose array does not spell `admin` — it was redirecting the owner off four tint routes. 🔴 **The silent-403 pattern is recorded as ONE systemic issue, not three bugs**: `fetchAll` returns `[]` on any failure so a 403 and "nothing here" are indistinguishable, made worse by `requireRole` failing with a 307 into an HTML page that arrives 200 — three separate silent failures in one week (manual-entry modal, missing-customer dropdowns, the old permissions grid). 🔴 **`prisma/seed.ts` has never heard of `user_page_access` or `isSuperuser`** (grep: zero hits), so a wipe-and-reseed would rebuild the fallback and leave live access empty. Plus `sub-areas` CREATE now being looser than its own EDIT (a parked owner decision, not drift), and a second unswept bypass — `api/reports/tint-summary:33`, which is a multi-clause GRANT rather than a redundant wrapper and still admits Operations User to a report every other tint endpoint now refuses him. The `mrn/photo` anchor corrected from `:169-170` to `:167-168`. Record: `docs/prompts/archive/2026-09/code-update-2026-09-06-tint-and-master-data.md`. Prior, v101 (2026-09-06): **§3 gains one rule: a status STRING is not an enum — import the module vocabulary, never retype the literal.** Most status columns here are plain Prisma `String`s and several carry no CHECK, so a wrong literal is never rejected, it silently matches nothing. `status: { not: "done" }` sat in FOUR live tint routes meaning "not finished" while the real value was `tinting_done` and `"done"` had never been written once — the predicate matched every row ever created (fixed 2026-09-05/06; `CLAUDE_TINT.md §1.4`). §7.3 needed no further edit: its `tint_assignments` status block was already corrected to the verified-live set in `a0f9378b` and nothing since has moved it. Schema stamp UNCHANGED at v27.23 — the Tint Manager board rebuild minted no schema version and every field it added to a payload reads a column that already existed. Prior, v100 (2026-09-04): **THE ACCESS MODEL CHANGED. §5 is rewritten and now opens with a red block: access comes from `user_page_access` (one row per user per page key), a job title is a LABEL and a STARTING TEMPLATE that grants nothing, and `admin` is `users.isSuperuser`. The `ACCESS_SOURCE` panic button — both SQL statements, copy-paste ready — is at the top of §5 because somebody who has never seen it needs to find it in five seconds.** Two schema versions minted: **v27.22** `user_page_access` (§7.14, 1,053 rows = 39 users × 27 keys, parity-checked against the live role system at 0 diffs, which is why the cutover changed nobody's access) and **v27.23** `users.isSuperuser` (§7.15, 1 of 39 users, checked as flag OR role). §12 gains an **Admin → Access** entry, §11 records that `buildNavItems` kept its rule and changed its source — so `navItems[0]` Home must now be re-derived per PERSON, not per role — and §13 gains three landmines: the two-arm superuser rule (one account, and dropping the role arm is the one change that cannot be undone from inside the app), that `role_permissions` must NOT be dropped while the switch can be flipped back because it IS the rollback, and the 22 remaining `role !== "admin"` bypass sites that a second superuser would fall through. Numbers derived from the tree and live SELECTs, not from the brief — which said ~54 `requireRole` calls (58), 21 bypass sites (22) and 2026-08-31 for commit `0f56eede` (2026-08-30). Prior, v99 (2026-09-04): **§7.13 gained a COVERAGE block and §13 a deferral entry; no schema change, so the schema stamp holds at v27.21.** The old §7.13 line said the audit log was "wired at mint — 3 routes … the other 67 are §13"; both numbers were true on 2026-09-01 and neither survived 1b and 1c. The replacement is **derived, not quoted** — `grep -rn "logAdminAction({" app/` returns **43 call sites in 43 files**, one per route, matching 3 (`f00808c1`) + 32 (`e757ba78`/`2207aca4`) + 8 (`c3cf726b`). It also records the two sets that are NOT wired and why they differ: **14** excluded permanently (old `sku_master` family + Shade Master + dispatch-cutoffs — they retire with their screens) and **13** deferred by decision into the Sampling Library / MRN / Tint / Billing conversions, so those files are not edited twice. §13 carries the same warning in the place a later session will actually hit it: an unwired route looks identical whether it was missed or parked, and these were parked. The conventions the 43 share — entity naming, `entityId: null` for bulk, changed-fields-only diffs, one row per bulk run, naming a hidden second write in the summary — are written down for the first time, because until now they lived only in the wired routes. Also recorded: the census's §6c list A says "48 files" and enumerates **49**, which makes the real gap total **71**, not 70 (43 + 14 + 13 + 1 machine path); the draft is left unedited, being a record of its own date. The v27.18 chain entry's "the other 67 remain" clause is marked as history rather than current state. Prior, v98 (2026-09-03): **v27.21 (the CI module: `ci_reason_master` + `ci_returns` + `ci_return_lines`, five CHECKs, `orders_invoiceNo_idx`) MINTED LATE and back-dated to 2026-08-31**, written as a record because it is the SECOND time a module reached production without entering the version chain; `docs/CLAUDE_CI.md` v1.0 is its authority and no §7 block is owed, per the v27.20 ruling for MRN. §5 and §12 gained rows for **both** CI and MRN — neither module was in either index. 🔴 §12 records that `/ci` is URL-only because a `PAGE_NAV_MAP` entry at index ≤ 2 would steal `floor_supervisor`'s Home button from `/picking`. Earlier, in v97 — v27.19 (MRN photos + the `closed` status + `mrn_photos`) and v27.20 (the delivery split: `mrn_lines.deliveryNo` and its per-delivery unique index) recorded in the chain ON THE DAY, which is what the v27.16/17 entry exists to say was not done last time; the "a full §7 block for MRN is still owed" claim retired — `docs/CLAUDE_MRN.md` is now the authority. Earlier in v96: `admin_audit_log` minted (§7.13); the MISSING v27.16/v27.17 MRN entries bridged into the version chain; §5 mail_orders grant corrected (tint_manager is view+edit, live SELECT); three §13 landmines closed or corrected by the Mail Orders write-route fix*
