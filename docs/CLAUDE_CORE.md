...
# v104 · Schema v27.24 · September 2026 · updated 2026-09-08 · Lives in: orbit-oms/docs/
...
**Permissions:** `lib/permissions.ts` has `PAGE_NAV_MAP`, the `PageKey` type, and `ALL_PAGE_KEYS` (**32 keys** since 2026-09-11 — `billing_picking`, the **Billing Picking tab** inside `/mail-orders`: the five `/api/billing/picking/*` routes gate on it (canView on list/marker/order, canEdit on mark-done/undo) and the tab's pill, body, count poll, Mark done and Undo all follow it. ⚠ NOT the floor board's `picking`. Deliberately **not** in `PAGE_NAV_MAP` — it is a tab, not a route. The Orders tab and `/api/billing/mail-order/actions`, `/ship-to-search`, `/dispatch-windows` stay on `mail_orders`. Known limit: a `billing_picking` holder without `mail_orders` still cannot open `/mail-orders` at all, since the layout guards `mail_orders` — the consolidated-shell session owns that). **Plus the FOUR billing action ticks** — `billing_hold` / `billing_slot` / `billing_urgent` / `billing_ship_to`, registered 2026-09-11, one per button on the Billing Orders tab so Slot can be granted without Hold. `canEdit` is their ONLY meaning; `canView` gates nothing on them and cannot be dashed (`isActionAvailable` returns true for View unconditionally). ⚠ **Nothing reads them yet** — all four buttons still gate on `mail_orders` canEdit inside `/api/billing/mail-order/actions`; the repoint is its own commit, registered first so people can be granted before the gate moves. They scope the BILLING face only: Floor sets the same four facts through its own routes on the `floor` key.
...
- **~~22 sites still compare `session.user.role !== "admin"` as an admin BYPASS of a permission check~~ — REMOVED 2026-09-06, and the warning attached to them was WRONG ON ITS OWN TERMS.** The 57 remaining one-clause wrappers (`if (!roles.includes("admin")) { checkAnyPermission(...) }` and its singular spelling) are gone; the check they wrapped now runs unconditionally. **The removal is provably behaviour-neutral, and not merely "neutral today":** `checkAnyPermission`'s first line is `if (roleSlugs.includes("admin")) return true` and `checkPermission`'s is `if (roleSlug === "admin") return true` — testing *the same value by the same test* the wrapper tested — and both then check `isSuperuser` immediately after. A wrapper could only ever skip a call that would have returned `true`. The commit proves it structurally: 266 lines removed, 152 added, and the 114-line difference is exactly 57 wrapper-opens plus 57 closing braces, with **not one line introduced** — every added line is a dedented copy of a removed one.
  ...
  - **Still open — TWO, not one, and the second is not harmless.** (a) `app/api/mrn/photo/[photoId]/route.ts:167-168` — `const isAdmin = roles.includes("admin"); const canDelete = isAdmin || (await checkAnyPermission(roles, "mrn", "canDelete"))`. Same redundancy, expression form rather than wrapper, so the sweep did not match it. Harmless for the same reason as the 57. *(This entry read `:169-170` until 2026-09-06; the lines are 167-168 — read them, do not trust the anchor.)* (b) `app/api/reports/tint-summary/route.ts:33` — a **MULTI-clause** bypass, `role !== "admin" && role !== OPERATIONS`, in front of `checkPermission(role, "tint_manager", "canView")`, behind a `requireRole([TINT_MANAGER, ADMIN, OPERATIONS, OPERATION_MANAGER])`. That is not a redundant bypass at all: it is a **permission grant written as a role name**, and it is the last of the 9 multi-clause survivors still standing after `d3211766` converted three and `fbbe30bd` converted five. It admits Operations User to the Tint Summary report while every other tint endpoint now refuses him — the one place the 2026-09-06 conversion did not reach, because `/api/reports/` is not `/api/tint/`.
