...
# v1.3 · Schema v27.24 · September 2026 · updated 2026-09-18
...
**Not Orbit's own trips.** The `trips` table (+ `trip_drops`, `trip_activity`), `/api/floor/trips/*` and `lib/trips/*` are a **separate system** — Orbit's own truck plan — that never writes `trip_report`. The two run side by side and nothing reconciles them; when NTS stops, this mirror is retired through the archive playbook and that module is unaffected (`app/api/floor/trips/route.ts:15-23`). Full collision table: **`CLAUDE_FLOOR_TRIPS.md` §2**.

**Downstream borrowers — "standalone" holds for code writes, not for meaning:**
- `lib/trips/drop-key.ts:3-5` cites §4's "Drops = unique customers" as Orbit's stop identity, so a change to §4's rule is a question for Orbit trips too.
- `scripts/backfill-nts-trips-2026-09-11.ts` (bf14cb51, 2026-09-11) **read** `trip_report` once (join `deliveryNo = orders.obdNumber`, `:164-186`) to create Orbit trips for bills already shipped on NTS trucks. One-time run.
- `app/api/floor/trips/[id]/dispatch/route.ts:21-22` names this mirror as "the record of the truck leaving" until the supervisor's loading screen exists.
...
Current visual facts: `/trips` has a **pale masthead** — `viewport.themeColor: "#F5F3FF"` and `appleWebApp.statusBarStyle: "default"` so iOS paints dark status-bar glyphs (`app/trips/page.tsx:12-37`, b51e6a15); the brand colour is **violet** (c96157ea); the trip sheet footer reads "Generated by Orbit" (`trip-sheet-document.tsx:508`, 8fac2a67).
...
⚠ **Code/doc mismatch, recorded 2026-09-18 — `fetchedAt`'s type.** This section says `TIMESTAMPTZ`;
`prisma/schema.prisma` (model `TripReport`) declares `fetchedAt DateTime @default(now())` with **no
`@db.Timestamptz`**, which Prisma reads as `timestamp(3)` without time zone. The live column type was
**not** checked in this pass, so which side is wrong is unverified. Same class as the missing index
`map:` above; no schema change made.
...
**Logo capture fix (2026-07-06):** the logo `<img>` used `width: "auto"` (only height set). `html-to-image`'s foreignObject serialization does **not** preserve intrinsic width on mobile WebKit specifically — the `navigator.share` path — so it silently collapsed to ~0 width (invisible). This failed identically for both a URL `src` and a base64 data URI, which is why an earlier data-URI attempt (442f249f) AND its revert (e2ddf3ff) both failed — neither fixed the width. **Fix (4a0001c4, 2026-07-06), two parts, both required:** the logo **is a data URI** — `src={JSW_DULUX_LOGO_DATA_URI}` from `lib/trip-report/logo-data-uri.ts` (`trip-sheet-document.tsx:216`), so capture never re-fetches the URL, which was itself flaky on mobile WebKit — **and** explicit `width: 141, height: 34` on the `<img>` (141 ≈ 34 × 800/193, same proportion as before — screen/PDF rendering unchanged), plus `cacheBust: false`. The code comment at `trip-sheet-document.tsx:218-223` says either alone fails. Confirmed working on a real phone.
...
| `lib/trip-report/logo-data-uri.ts` | `JSW_DULUX_LOGO_DATA_URI` — auto-generated base64 of `public/jsw-dulux-logo.png`, the sheet's logo `src` (§6) |
| `components/trip-report/trip-sheet-print-button.tsx` | `TripSheetPrintButton` — "Print / Save PDF", calls `window.print()`; used by `app/trips/[tripNo]/sheet/page.tsx:96` |
...
## Change log — v1.3 (2026-09-18, canon sweep reconciliation)

- TRP-15 (§1): disambiguation from Orbit's own trips (`trips` / `/api/floor/trips` / `lib/trips`), pointing to `CLAUDE_FLOOR_TRIPS.md` §2; downstream borrowers recorded (`lib/trips/drop-key.ts`, the bf14cb51 backfill, the dispatch route header).
- TRP-16 (§1): current visual facts — pale masthead (b51e6a15), violet (c96157ea), "Generated by Orbit" (8fac2a67). Ownership line unchanged.
- TRP-17 (§3): `fetchedAt` TIMESTAMPTZ vs Prisma's plain `DateTime` recorded as a code/doc mismatch; live type unverified; no schema change.
- TRP-18 (§6): logo wording — the live logo IS a data URI (4a0001c4), and both the data URI and the explicit width are required.
- TRP-19 (§8): `lib/trip-report/logo-data-uri.ts` and `components/trip-report/trip-sheet-print-button.tsx` added.
- Schema stamp stays v27.24 — re-reconciled against CORE v27.24 in this pass.
...
*Trip Report v1.3 · Schema v27.24 · OrbitOMS · updated 2026-09-18 — canon sweep: §1 now separates this mirror from Orbit's own trips (`CLAUDE_FLOOR_TRIPS.md` §2) and names the mirror's downstream borrowers; logo, key files, visual facts and the `fetchedAt` type mismatch recorded. Prior, v1.2 (2026-09-08): the v27.24 mirror rewrite — `sourceId` does NOT change per pull (§2.1), `fetchedAt` is no longer a liveness signal (`mirror_heartbeat."lastRunAt"` is, §7), and the host running the live puller is UNKNOWN (§7).*
