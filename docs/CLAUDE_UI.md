...
# v5.35 · September 2026 · updated 2026-09-24 · No Schema stamp BY DESIGN (decided 2026-08-04) — this file tracks components, not tables · Lives in: orbit-oms/docs/
...
| **`data.*`** | `teal` `#0D9488` · `blue` `#2563EB` · `orange` `#EA580C` · `rose` `#E11D48` · `cyan` `#0891B2` · `lime` `#65A30D` · `pink` `#DB2777` · `slate` `#475569` · `brown` `#8B5A2B` | — | **IDENTITIES, never states. Never reassign a shipped data colour.** `data.rose` shares `danger`'s hex by a recorded decision — do not "resolve" it. `data.slate` is spoken for (§59.9). **`data.blue` = the Local delivery type ONLY** (the Telephonic CI tag used it 2026-09-23 → 24 and moved off). **CI = SOLID `ink-900`** (`bg-ink-900 text-white`) everywhere in billing — never the pale ink chip, which read as disabled. **`data.brown` · Hand** (the dealer collects; owner 2026-09-24) — 5.84:1 on white, 5.31:1 on its pale tint. **After brown, no `data.*` colour is free — the next category needs a new token.** |
