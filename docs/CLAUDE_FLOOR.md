...
- **Other routes.** Every route of the tab that is in **no club**, plus the route-less bills, share **ONE "Other routes" card** (2026-09-19) — one line each, lines **by kilos due, highest first** (name breaks ties), **"No route" always the last line**. It is **display-only**: nothing is written to `route_clubs` / `route_club_members` for it (Parvat stays unclubbed in the database); `buildRouteCards` builds it from the rows every render (`otherCard`, key `other`). It looks and opens exactly like a club card, and it is **always the last card**. No card when nothing is unclubbed; one line when only one route has bills.
- **The grid.** One grid of equal cards that flow in order: **4 per row at ≥1470px, 3 at 1100–1469px, 2 below** (`useCardColumns`, `matchMedia` change events — not a key listener). Order: **clubs by `sortOrder`, then Other routes, always last.** Today at 1920px that is one row: Adajan + Olpad · Ghod Dod + Udhana · Varachha + Kamrej · Other routes (Parvat, then No route). An empty club keeps its place, dimmed, reading "No bills". The column count is known to JS because a panel opens under its card's ROW. ⚠ 1470, not 1400: four columns at 1440px cut "Kamrej" to "Ka…" (the line needs 225px; measured in Chrome with the app's font).
- **Same size.** Every card is the same width and the **same height across the whole board**: the head (name, big kilos, "N stops · L") plus as many equal line slots as the card with the most routes. Spare slots are invisible spacers between the head and the card's own lines, so the head stays at the top and the last bar sits on the bottom edge. An Other routes card of 3+ lines grows every card with it, exactly as a 3-route club would. Text never wraps; at worst a route name ellipsises.
