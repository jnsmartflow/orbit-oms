...
// ── "not in master" marker ──────────────────────────────────────────────────
// (2026-08-31) The dealer's ship-to code is not in `delivery_point_master`.
//
// 🔴 IT IS A DATA-QUALITY NOTE, NOT AN URGENCY SIGNAL. The bill is pickable and
// nothing about it is wrong — the depot's master data is simply missing a row.
// That is why it takes AgeBadge's QUIETEST tier (`bg-amber-50 text-amber-700`,
// the "1d — yesterday is not a crisis" nudge) and never the 2d+/4d+ solids.
// Whoever restyles this: amber-100/300 and red belong to staleness, and putting
// this on that ladder claims an urgency the data does not support — exactly the
// argument AgeBadge already makes for `noDispatchDate` going grey.
//
// ⚠ NEVER TEAL. Teal on this board is the Assign CTA and the selected-card
// tint, and one-teal-per-card is CLAUDE_UI.md §62's rule.
//
// ⚠ NO SECOND DOT. `RouteDot` sits immediately left of this on both cards, it
// keys on `deliveryType`, and it already renders grey (#9ca3af) on the null
// that comes with an unmastered dealer. A coloured dot here would be a second
// status light on one 12px line.
//
// Geometry is AgeBadge's, to the pixel (10.5px/600, px-2 py-[3px], rounded-full,
// shrink-0, whitespace-nowrap) — CLAUDE_UI.md §60's chip token. No new spacing
// scale was invented, for the same reason SmuBadge did not invent one.
//
// ⚠ `onDark` COVERS TWO GROUNDS, and that is deliberate: the solid-red
// duplicate-SO card AND the teal detail header. Both need the white-pill flip
// that duplicate-so-tag.tsx documents ("each of those has to flip to a WHITE
// pill … or it vanishes into the fill"). It is NOT `DUP_SO_BADGE_CLASS` though
// — that pill's text is #b91c1c, and wearing the duplicate-SO red would make
// this read as part of that treatment rather than as its own note. White +
// amber-700 keeps the register on both grounds.
export function NotInMasterChip({ onDark = false }: { onDark?: boolean }): React.JSX.Element {
  return (
    <span
      title="This dealer is not in the delivery-point master — the name shown comes from the SAP bill"
      aria-label="Dealer not in master — name shown comes from the SAP bill"
      className={
        "text-[10.5px] font-semibold px-2 py-[3px] rounded-full shrink-0 whitespace-nowrap " +
        (onDark ? "bg-white text-amber-700" : "bg-amber-50 text-amber-700")
      }
    >
      not in master
    </span>
...
/**
 * Does the CARD's where-row show the marker instead of its em-dash?
 *
 * 🔴 BOTH HALVES MATTER. `!dealerInMaster` alone is not the test: if a bill
 * somehow carries a route while its dealer is unmastered, the ROUTE wins — a
 * real lane name is worth more to a supervisor than a note about master data,
 * and the marker is only ever a REPLACEMENT for the bare `—`, never an addition
 * to the row. That keeps the swap width-neutral in the only direction that
 * matters at 320px (CLAUDE_UI.md §60).
 *
 * Exported so both cards ask the same question. The DETAIL HEADERS deliberately
 * do NOT use this — up there the marker sits beside the name, not in a route
 * slot, so its gate is the bare `!dealerInMaster`.
 */
export function showsRouteSlotMarker(
  row: Pick<PickingQueueRow, "dealerInMaster" | "route">,
): boolean {
  return !row.dealerInMaster && row.route === null;
}
