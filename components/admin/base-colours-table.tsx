...
export interface BaseColourRow {
  id:       number;
  name:     string;
  isActive: boolean;
  _count:   { skus: number };
...
function buildForm(row: BaseColourRow | null): typeof EMPTY_FORM {
...
export function BaseColoursTable({ initialRows }: Props) {
  const [rows,        setRows]        = useState<BaseColourRow[]>(initialRows);
  const [sheetOpen,   setSheetOpen]   = useState(false);
  const [editTarget,  setEditTarget]  = useState<BaseColourRow | null>(null);
  ...
  function openEdit(row: BaseColourRow) {
    setEditTarget(row);
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      ...
      const res = editTarget
        ? await fetch(`/api/admin/base-colours/${editTarget.id}`, {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(body),
          })
        : await fetch("/api/admin/base-colours", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(body),
          });
      ...
      if (!res.ok) {
        if (res.status === 409) {
          setFieldErrors({ name: "A base colour with this name already exists." });
      ...
      if (editTarget) {
        ...
        toast.success("Base colour updated.");
      } else {
        setRows((prev) => [...prev, data].sort((a, b) => a.name.localeCompare(b.name)));
        toast.success(`Base colour "${data.name}" created.`);
      }
      setSheetOpen(false);
    } catch {
  ...
  async function handleToggle(row: BaseColourRow) {
    const newActive = !row.isActive;
    try {
      const res = await fetch(`/api/admin/base-colours/${row.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
      ...
      toast.success(newActive ? "Base colour activated." : "Base colour deactivated.");
    } catch {
      toast.error("Network error.");
    }
  }
  ...
  function handleTemplateDownload() {
    ...
    a.href = url;
    a.download = "template-base-colours.csv";
    a.click();
    URL.revokeObjectURL(url);
  }

  async function handleImportConfirm(validRows: Record<string, string>[]) {
    const res = await fetch("/api/admin/base-colours/import", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-lg font-bold text-brand-700">Base Colours</h1>
        <div className="flex gap-2">
          ...
          <Button size="sm" onClick={openAdd} className="bg-brand-600 hover:bg-brand-700 text-white">+ Add Base Colour</Button>
        </div>
            ...
            {rows.length === 0 && (
              <TableRow>
                <TableCell colSpan={4} className="text-center text-gray-500 py-8">
                  No base colours configured yet.
                </TableCell>
              </TableRow>
            )}
            {rows.map((row) => (
              <TableRow key={row.id}>
                <TableCell className="font-medium text-gray-800">
                  {row.name}
                  {row.name === "N/A" && (
                    <span className="ml-2 text-xs text-gray-400">(fallback, cannot deactivate)</span>
                  )}
                </TableCell>
                ...
                <TableCell className="text-right">
                  <div className="flex items-center justify-end gap-2">
                    ...
                    <Button
                      ...
                      disabled={row.name === "N/A" && row.isActive}
                      title={row.name === "N/A" ? "N/A cannot be deactivated" : undefined}
      ...
      <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
        <SheetContent className="w-full sm:max-w-2xl overflow-y-auto">
          <SheetHeader>
            <SheetTitle>{editTarget ? "Edit Base Colour" : "Add Base Colour"}</SheetTitle>
          </SheetHeader>
          ...
          <form onSubmit={handleSave} className="oa-sheet-form flex flex-col gap-5 px-6 pb-0">
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                placeholder="e.g. White Base, Deep Base, Clear…"
              />
            ...
            <div className="flex items-center justify-between py-3 px-4 rounded-lg border border-[#e5e7eb] bg-gray-50">
              <div>
                ...
                <div className="text-xs text-gray-500 mt-0.5">
                  {editTarget?.name === "N/A"
                    ? "N/A is the fallback for non-tint SKUs and cannot be deactivated"
                    : "Inactive colours are hidden from SKU forms"}
                </div>
              </div>
              <Switch
                checked={form.isActive}
                onCheckedChange={(v) => setField("isActive", v)}
                disabled={editTarget?.name === "N/A"}
            ...
            <div className="sticky bottom-0 bg-white border-t border-[#e5e7eb] -mx-6 px-6 py-4 flex gap-3 mt-6">
              ...
              <Button type="submit" className="flex-1 h-10 text-sm bg-brand-600 hover:bg-brand-700 text-white rounded-lg font-semibold" disabled={saving}>{saving ? "Saving…" : editTarget ? "Save Changes" : "Create Base Colour"}</Button>
            </div>
          </form>
        </SheetContent>
      </Sheet>
