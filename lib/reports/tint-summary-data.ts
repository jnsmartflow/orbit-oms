...
//
// Owns ALL of the read-only aggregation. Consumed by:
//   - app/api/reports/tint-summary/route.ts  (JSON endpoint)
//   - app/reports/tint-summary/page.tsx      (server-rendered print page)
// Auth lives in the callers; this module is pure data + force-dynamic-safe
// (every call hits the DB live). CORE §3: no $transaction, camelCase, UTC→IST.
//
// "Today" axes (locked):
...
//
// Respects hide rules: every base query AND-merges getHideExclusion() — never
// bypassed. The operators filter applies ONLY to operator-centric outputs
// (operators[], completedRegister, openRegister); aggregate balances ignore it
// so opening-balance arithmetic stays internally consistent. smu/area/includeHold
// apply to every order-based section.
// ─────────────────────────────────────────────────────────────────────────────

export interface TintSummaryParams {
  /** YYYY-MM-DD (IST OBD-date). Invalid/absent → today IST. */
  date?: string;
  /** Operator ids — scopes operator-centric outputs only. */
  operators?: number[];
  /** false → drop lower(dispatchStatus)="hold" OBDs. Default true. */
  includeHold?: boolean;
  /** SMU names — filters all order-based sections. */
  smu?: string[];
  /** Delivery-type names — filters all order-based sections. */
  area?: string[];
  /** Intake-vs-completed trend window length. Default 7, clamped 1..60. */
  trendDays?: number;
}
...
export async function getTintSummaryData(params: TintSummaryParams = {}): Promise<TintSummaryData> {
  const reportDate = params.date && /^\d{4}-\d{2}-\d{2}$/.test(params.date)
    ? params.date
    : istDateStr(new Date());

  const operators = params.operators ?? [];
  const includeHold = params.includeHold ?? true;
  const smuFilterArr = params.smu ?? [];
  const areaFilterArr = params.area ?? [];
  const trendDaysRaw = params.trendDays ?? 7;
