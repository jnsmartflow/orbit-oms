...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    await prisma.order_status_logs.create({
      data: {
        orderId:     order.id,
        // The bill's REAL prior stage (2026-09-11). It was hardcoded
        // "pending_support", which was true while that was the only pullable
        // stage and is a lie now that `pending_picking` is one too — and a
        // wrong `fromStage` is exactly what makes an audit trail useless.
