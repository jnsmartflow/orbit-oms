import { NextResponse } from "next/server";
import { auth } from "@/lib/auth";
import { requireSuperuser } from "@/lib/rbac";
import { prisma } from "@/lib/prisma";
import { logAdminAction } from "@/lib/audit/log";

export const dynamic = "force-dynamic";

export async function POST(req: Request) {
  const session = await auth();
  requireSuperuser(session);

  const body = await req.json() as { rows: Record<string, string>[] };
  const rows = body.rows ?? [];

  if (rows.length === 0) {
    return NextResponse.json({ imported: 0, skipped: 0, failed: 0, errors: [] });
  }

  const areas = await prisma.area_master.findMany({ select: { id: true, name: true } });
  const areaMap = new Map(areas.map((a) => [a.name.toLowerCase(), a.id]));

  const errors: { row: number; reason: string }[] = [];
  const data: { name: string; areaId: number; isActive: boolean }[] = [];

  rows.forEach((r, i) => {
    const name   = r.name?.trim();
    const areaName = r.area?.trim();
    const areaId   = areaMap.get(areaName?.toLowerCase() ?? "");

    if (!name)   { errors.push({ row: i + 2, reason: "Name is required." }); return; }
    if (!areaId) { errors.push({ row: i + 2, reason: `Area "${areaName}" not found.` }); return; }

    data.push({ name, areaId, isActive: true });
  });

  const result = await prisma.sub_area_master.createMany({
    data,
    skipDuplicates: true,
  });

  const imported = result.count;
  const skipped  = data.length - imported;

  // ONE line for the whole upload (audit RULE 6).
  await logAdminAction({
    userId: parseInt(session!.user.id, 10),
    entity: "sub_areas",
    entityId: null,
    action: "import",
    summary:
      `imported ${imported} sub-area(s) (${skipped} skipped, ${errors.length} rejected) ` +
      `from ${rows.length} row(s)`,
    after: { rows: rows.length, imported, skipped, rejected: errors.length },
  });

  return NextResponse.json({ imported, skipped, failed: errors.length, errors });
}
