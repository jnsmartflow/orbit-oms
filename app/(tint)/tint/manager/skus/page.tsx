...
export default async function TintManagerSkusPage() {
  const session = await auth();
  if (!session?.user) redirect("/login");
  const canView = await checkPermission(session.user.role, "skus", "canView");
  if (!canView) redirect("/unauthorized");
  const perms = await getPagePermissions(session.user.role, "skus");

  const [skus, total, categories, productNames, baseColours] = await Promise.all([
    prisma.sku_master.findMany({ take: 25, orderBy: { skuCode: "asc" }, include }),
