  ...
  const billingHeaderSlot = billingV2 ? (
    <>
      ...
      {(() => {
        ...
        // PRINT TAB (slice 9, owner, option B): the date stays — it picks the day
        // of the Copied section — and Filter + shortcuts, which do nothing on
        // Print, are still RENDERED but INVISIBLE. They hold their width, so the
        // right-aligned date control sits exactly where it does on Orders
        // instead of sliding right. `visibility: hidden` (inherited through the
        // `display: contents` wrapper, which adds no box) also takes every
