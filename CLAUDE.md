...
| `/ci` — **CI, Goods Return Note**: stock coming BACK from a customer, the return counterpart to MRN. `ci_returns` / `ci_return_lines` / `ci_reason_master`, `lib/ci/*`, `/api/ci/*`. TWO faces on one route, branched by ROLE: floor_supervisor's phone (search a bill → Full bill/Part → lines → details → submit) and billing's desk (one rail, pending above closed → Close CI). ⚠ **No `CLAUDE_CI.md` exists yet** — the design record is `docs/prompts/drafts/web-update-2026-08-31-ci-module.md` (v3.3) plus `docs/mockups/ci/{supervisor,billing}.html`, and a §7 block in CORE is still owed | `docs/prompts/drafts/web-update-2026-08-31-ci-module.md` (+ `docs/CLAUDE_CORE.md §3` for the engineering rules it leans on) |
